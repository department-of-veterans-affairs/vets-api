...
module V0
  class LettersDiscrepancyController < ApplicationController
    ...
    def index
      ...
      lh_response = lh_service.get_eligible_letter_types('1012845630V900607') # @current_user.icn
      # Format response into lh_letters array
      lh_letters = lh_response[:letters].map { |letter| letter[:letterType] }

      # Call evss endpoint
      evss_response = evss_service.get_letters
      # Format response into evss_letters array
      evss_letters = evss_response.letters.map { |letter| letter.letter_type }

      # Return if there are no differences
      render json: [] if lh_letters.sort == evss_letters.sort

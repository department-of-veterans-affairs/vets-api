...
describe VBADocuments::MonthlyStat, type: :model do
  describe 'validations' do
    context 'when constructed with a valid month and year' do
      let(:monthly_stat) { FactoryBot.build(:monthly_stat, month: 12, year: 2023) }
    ...
    context 'when constructed with an invalid month' do
      let(:monthly_stat) { FactoryBot.build(:monthly_stat, month: 'December', year: 2023) }
    ...
    context 'when constructed with an invalid year' do
      let(:monthly_stat) { FactoryBot.build(:monthly_stat, month: 12, year: 23) }
    ...
    context 'when a report already exists for the month and year' do
      let(:monthly_stat) { FactoryBot.build(:monthly_stat, month: 1, year: 2023) }

      before { FactoryBot.create(:monthly_stat, month: 1, year: 2023) }

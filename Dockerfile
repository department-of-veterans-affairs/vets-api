...
RUN apt-get update -y && apt-get install -y ca-certificates-java && update-ca-certificates -f && dpkg --configure -a && apt-get install -y build-essential libpq-dev git imagemagick curl wget ca-certificates-java pdftk file \
  && apt-get clean \

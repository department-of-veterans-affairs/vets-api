...
RSpec.describe UnifiedHealthData::Adapters::OracleHealthPrescriptionAdapter do
  ...
  describe 'V2_STATUS_GROUPS constant' do
    it 'defines correct status groupings' do
      ...
      expect(UnifiedHealthData::Adapters::V2StatusMapper::V2_STATUS_GROUPS).to eq(expected_groups)
    end
  ...
  describe '#map_to_v2_status' do
    context 'with Active statuses' do
      it 'maps Active to Active' do
        expect(adapter.map_to_v2_status('Active')).to eq('Active')
      end

      it 'maps Active: Parked to Active' do
        expect(adapter.map_to_v2_status('Active: Parked')).to eq('Active')
      end

      it 'maps Active: Non-VA to Active' do
        expect(adapter.map_to_v2_status('Active: Non-VA')).to eq('Active')
      end
    end

    context 'with In progress statuses' do
      it 'maps Active: Submitted to In progress' do
        expect(adapter.map_to_v2_status('Active: Submitted')).to eq('In progress')
      end

      it 'maps Active: Refill in Process to In progress' do
        expect(adapter.map_to_v2_status('Active: Refill in Process')).to eq('In progress')
      end
    end

    context 'with Inactive statuses' do
      it 'maps Expired to Inactive' do
        expect(adapter.map_to_v2_status('Expired')).to eq('Inactive')
      end

      it 'maps Discontinued to Inactive' do
        expect(adapter.map_to_v2_status('Discontinued')).to eq('Inactive')
      end

      it 'maps Active: On hold to Inactive' do
        expect(adapter.map_to_v2_status('Active: On hold')).to eq('Inactive')
      end
    end

    context 'with other statuses' do
      it 'maps Transferred to Transferred' do
        expect(adapter.map_to_v2_status('Transferred')).to eq('Transferred')
      end

      it 'maps Unknown to Status not available' do
        expect(adapter.map_to_v2_status('Unknown')).to eq('Status not available')
      end
    end
    ...
    context 'with edge cases' do
      it 'returns Status not available for nil' do
        expect(adapter.map_to_v2_status(nil)).to eq('Status not available')
      end

      it 'returns Status not available for blank string' do
        expect(adapter.map_to_v2_status('')).to eq('Status not available')
      end

      it 'returns Status not available for unmapped statuses' do
        expect(adapter.map_to_v2_status('Some Random Status')).to eq('Status not available')
      end
    end
  end
  ...
  describe '#original_statuses_for_v2_status' do
    it 'returns original statuses for Active' do
      expect(adapter.original_statuses_for_v2_status('Active')).to eq(
        ['Active', 'Active: Parked', 'Active: Non-VA']
      )
    end

    it 'returns original statuses for In progress' do
      expect(adapter.original_statuses_for_v2_status('In progress')).to eq(
        ['Active: Submitted', 'Active: Refill in Process']
      )
    end

    it 'returns original statuses for Inactive' do
      expect(adapter.original_statuses_for_v2_status('Inactive')).to eq(
        ['Expired', 'Discontinued', 'Active: On hold']
      )
    end

    it 'returns original statuses for Transferred' do
      expect(adapter.original_statuses_for_v2_status('Transferred')).to eq(['Transferred'])
    end

    it 'returns original statuses for Status not available' do
      expect(adapter.original_statuses_for_v2_status('Status not available')).to eq(['Unknown'])
    end

    it 'returns empty array for unknown V2 status' do
      expect(adapter.original_statuses_for_v2_status('Unknown V2 Status')).to eq([])
    end

    it 'returns empty array for nil' do
      expect(adapter.original_statuses_for_v2_status(nil)).to eq([])
    end
  end
  ...
  describe '#apply_v2_status_mapping' do
    ...
    it 'maps disp_status to V2 status' do
      result = adapter.apply_v2_status_mapping(prescription)
    ...
    it 'handles prescription without disp_status method' do
      ...
      result = adapter.apply_v2_status_mapping(plain_object)
    ...
    it 'maps all status types correctly' do
      ...
      status_mappings.each do |original, expected|
        ...
        adapter.apply_v2_status_mapping(rx)
  ...
  describe '#apply_v2_status_mapping_to_collection' do
    ...
    it 'maps all prescriptions to V2 statuses' do
      result = adapter.apply_v2_status_mapping_to_collection(prescriptions)
    ...
    it 'handles empty array' do
      result = adapter.apply_v2_status_mapping_to_collection([])
      expect(result).to eq([])
    end

    it 'mutates original prescription objects' do
      original_first = prescriptions.first
      adapter.apply_v2_status_mapping_to_collection(prescriptions)

      expect(original_first.disp_status).to eq('Active')
    end
  end

  # ...existing code for other adapter tests...
end

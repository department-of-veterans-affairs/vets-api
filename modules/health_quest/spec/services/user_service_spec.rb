...
describe HealthQuest::UserService do
  ...
  before do
    allow(HealthQuest::Configuration.instance).to receive(:rsa_key).and_return(rsa_key)
    ...
    describe '#extend_session' do
      ...
      context 'with one call inside the original lock (< 60s)' do
        it 'does not trigger the extend session job' do
          VCR.use_cassette('health_quest/users/get_user_jwts') do
            expect(HealthQuest::ExtendSessionJob).not_to receive(:perform_async).with(user.account_uuid)
      ...
      context 'with one call outside the lock (> 60s)' do
        it 'triggers the extend session job' do
          VCR.use_cassette('health_quest/users/get_user_jwts') do
            ...
            expect(HealthQuest::ExtendSessionJob).to receive(:perform_async).with(user.account_uuid).once
            subject.extend_session(user.account_uuid)
          end
        end
      end

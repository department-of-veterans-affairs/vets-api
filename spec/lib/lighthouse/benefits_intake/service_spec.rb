...
RSpec.describe BenefitsIntake::Service do
  ...
  describe '#perform_upload' do
    ...
    before do
      ...
      allow(Common::FileHelpers).to receive(:generate_clamav_temp).and_return 'a-temp-file'
    end
    ...
    it 'performs the upload' do
      ...
      expect(Common::FileHelpers).to(
        receive(:generate_clamav_temp).once.with(metadata.to_json, 'api.benefits_intake.uuid.metadata.json')
      )
    ...
    it 'errors on invalid JSON metadata' do
      ...
      expect(Common::FileHelpers).not_to receive(:generate_clamav_temp)

...
module BEP
  class DependentV2Service
    ...
    # Retrieve dependents for the veteran from BEP.
    # Returns a Hash with :persons => [] (array) for consistent downstream processing.
    # Falls back to an empty response when participant_id is blank or the BEP response
    ...
    # - Submits PDFs to ClaimsEvidenceApi (VBMS) and then attempts standard BEP submission
    # - If PDF submission fails, falls back to central submission flow
    # Returns a hash containing the enqueued job id for the BEP submit job when applicable.
    def submit_686c_form(claim)
      # Set email for BEP service and notification emails from form email if va_profile_email is not available
      ...
      InProgressForm.find_by(form_id: BEP::SubmitForm686cV2Job::FORM_ID, user_uuid: uuid)&.submission_processing!
    ...
    # Construct the BEP service client for subsequent remote calls.
    # Uses the veteran's ICN and external_key for identification.
    def service
      @service ||= BEP::Services.new(external_uid: icn, external_key:)
    end

    # Centralized BEP error logging/tracking wrapper for visibility in monitoring.
    # Emits a monitor event with sanitized error details and increments metrics when configured.
    def log_bep_errors(error)
      increment_non_validation_error(error) if Flipper.enabled?(:va_dependents_bep_extra_error_logging)

      # Temporarily logging a few iterations of status code to see what BEP returns in the error
      @monitor.track_event(
        'warn', 'BEP::DependentService#submit_686c_form method failed!',
        "#{STATS_KEY}.failure",
        {
          error: error.message,
    ...
    # This isolates transient BEP HTTP status conditions for easier alerting.
    def increment_non_validation_error(error)
      ...
      if error_type.present?
        ...
        StatsD.increment("#{STATS_KEY}.non_validation_error.#{error_status}", tags: ["form_id:#{BEP::SubmitForm686cV2Job::FORM_ID}"])
      end
    end
    ...
    def submit_pdf_job(claim:)
      @monitor = init_monitor(claim&.id)
      @monitor.track_event('info', 'BEP::DependentV2Service#submit_pdf_job called to begin ClaimsEvidenceApi::Uploader',
      ...
      @monitor.track_event('info', 'BEP::DependentV2Service#submit_pdf_job completed',
      ...
      @monitor.track_event('warn',
                           'BEP::DependentV2Service#submit_pdf_job failed, submitting to Lighthouse Benefits Intake',
    ...
    def submit_attachments_via_claims_evidence(form_id, claim)
      Rails.logger.info("BEP::DependentV2Service claims evidence upload of #{form_id} claim_id #{claim.id} attachments")
    ...
    # Enqueue the appropriate Sidekiq job to submit to the standard BEP service (686c or 674).
    def submit_to_standard_service(claim:, encrypted_vet_info:)
      if claim.submittable_686?
        BEP::SubmitForm686cV2Job.perform_async(
          uuid, claim.id, encrypted_vet_info
        )
      else
        BEP::SubmitForm674V2Job.perform_async(
          uuid, claim.id, encrypted_vet_info
        )
      end
    end
    ...
    def submit_to_central_service(claim:, encrypted_vet_info:)
      ...
      user = BEP::SubmitForm686cV2Job.generate_user_struct(vet_info)
    ...
    # Build an external key used for BEP client identification.
    ...
    # Retrieve veteran identifiers from BEP and compose the hash used in 686C submissions.
    # - Attempts lookup by participant_id then SSN
    # - Extracts and normalizes va file number when present
    # - Logs and continues on BEP failures
    # Additional notes on this method can be found in app/services/bep/dependents_veteran_identifiers.md
    def get_form_hash_686c
      begin
        ...
        if bep_person.respond_to?(:[]) && bep_person[:file_nbr].present?
          @file_number = bep_person[:file_nbr]
        else
          @monitor.track_event('warn',
                               'BEP::DependentV2Service#get_form_hash_686c missing bep_person file_nbr',
                               "#{STATS_KEY}.file_number.missing",
                               { bep_person_present: bep_person.present? ? 'yes' : 'no' })
          @file_number = nil
        end
    ...
    # Lookup BEP person record by participant_id (preferred) or SSN (fallback)
    def lookup_bep_person
      bep_person = service.people.find_person_by_ptcpnt_id(participant_id, ssn)

      if bep_person.present?
        @monitor.track_event('info', 'BEP::DependentV2Service#get_form_hash_686c found bep_person by PID',
                             "#{STATS_KEY}.find_by_participant_id")
      else
        bep_person = service.people.find_by_ssn(ssn) # rubocop:disable Rails/DynamicFindBy
        @monitor.track_event('info', 'BEP::DependentV2Service#get_form_hash_686c found bep_person by ssn',
    ...
    def generate_hash_from_details
      ...
      # Sometimes BEP will return a 502 (Bad Gateway) when trying to find a person by participant id or ssn.
    ...
    def get_user_email(user)
      ...
      @monitor.track_event('warn', 'BEP::DependentV2Service#get_user_email failed to get va_profile_email',

    ...
    module HigherLevelReview::V3
      class Structure
        ...
        def fill_benefit_type_fields
          FormData::BENEFIT_TYPE_CODES.keys.reduce({}) do |data, benefit_type|
            data.merge!({ form_fields.benefit_type_field(benefit_type) => form_data.benefit_type_code(benefit_type) })
          end
        end

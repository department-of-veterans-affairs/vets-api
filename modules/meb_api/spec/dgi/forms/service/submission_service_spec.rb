...
RSpec.describe MebApi::DGI::Forms::Submission::Service do
  ...
  describe '#update_dd_params (private method)' do
    ...
    context 'when account number does not contain asterisks (not masked)' do
      ...
      it 'does not replace the values' do
        result = service.send(:update_dd_params, params_without_masked_values, payment_account_data)

        expect(result[:form][:direct_deposit][:direct_deposit_account_number]).to eq('9876543210')
        expect(result[:form][:direct_deposit][:direct_deposit_routing_number]).to eq('021000021')
      end

      it 'does not replace values even when dd_params is nil' do
        result = service.send(:update_dd_params, params_without_masked_values, nil)

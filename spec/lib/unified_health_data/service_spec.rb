...
describe UnifiedHealthData::Service, type: :service do
  ...
  describe '#get_labs' do
    ...
    before do
      allow(service).to receive(:fetch_access_token).and_return('token')
      allow(service).to receive(:perform).and_return(double(body: labs_response))
      allow(service).to receive(:parse_response_body).and_return(labs_response)
    end

    it 'returns labs/tests when Flipper is enabled for both codes' do
    ...
    it 'filters out labs/tests when Flipper is disabled' do
  ...
  describe '#fetch_ordered_by' do
    it 'returns practitioner full name if present' do
      record = { 'resource' => { 'contained' => [{ 'resourceType' => 'Practitioner', 'name' => [{ 'given' => ['A'], 'family' => 'B' }] }] } }

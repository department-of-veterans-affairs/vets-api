...
class UpstreamSettingsSync
  ...
  def run
    ...
    puts "Syncing #{@options[:namespace]} settings (excluding #{@options[:exclusions].size} parameters)" if @options[:exclusions].any?
  ...
  def filter_parameters(parameters)
    ...
    puts "Found #{original_count} parameters, excluding #{excluded_count}, processing #{filtered.size}" unless @options[:dry_run]
  ...
  def update_single_setting(setting_path, new_value)
    ...
    lines.each_with_index do |line, index|
      ...
      if line.match(/^(\s*)([^:\s#]+):\s*([^#]*)(#.*)?$/)
        ...
        if in_namespace && key == setting_key
          ...
          lines[index] = "#{$1}#{key}: #{formatted_value}#{comment_part}\n"
          line_updated = true
          break
        end
      end
    ...
    unless line_updated
      lines = add_new_setting_to_lines(lines, setting_path, new_value)
    end
  ...
  def should_update_setting?(key, current_value, new_value)
    if current_value.nil?
      ...
      if @options[:force]
        ...
        return prompt_for_update(key, current_value, new_value)
      end
    else

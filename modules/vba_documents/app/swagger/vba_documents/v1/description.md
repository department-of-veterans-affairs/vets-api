...
## Background 
This API provides a secure, efficient, and tracked alternative to mail or fax for VA benefit claim document submissions. Documents are uploaded directly to the VBA system, where they can then be processed. Compared to mail or fax, these documents are processed faster, ultimately providing Veterans with claim decisions more quickly. 

All successfully submitted documents are routed to the correct office for processing. 
...
### Date of receipt
The date and time that documents are successfully submitted through the Benefits Intake API is used as the official VA date of receipt. However, note that until a document status of `received`, `processing`, `success`, or `vbms` is returned, a client cannot consider the document received by VA. 
...
### Test data
We use mock test data in the sandbox environment. Data is not sent downstream and does not necessary to align with any other systems' data. 

...
RSpec.describe 'Income and Assets End to End', skip: 'TODO after schema built', type: :request do
  let(:form) { build(:employment_questionnaires_claim) }
  ...
  before do
    ...
    allow(Flipper).to receive(:enabled?).with(:employment_questionnaires_submitted_email_notification).and_return true
  ...
  it 'successfully completes the submission process' do
    # form submission
    expect(EmploymentQuestionnaires::SavedClaim).to receive(:new).with(form: form.form).and_call_original
    ...
    post '/employment_questionnaires/v0/claims', params: { param_name => { form: form.form } }
    ...
    employment_questionnaires_claim = EmploymentQuestionnaires::SavedClaim.find(saved_claim_id)
    expect(employment_questionnaires_claim).to be_present
    expect(employment_questionnaires_claim.confirmation_number).to eq data['attributes']['confirmation_number']
    ...
    email = EmploymentQuestionnaires::NotificationEmail.new(saved_claim_id)
    allow(EmploymentQuestionnaires::NotificationEmail).to receive(:new).and_return(email)
    ...
    lh_bi_uuid = EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob.new.perform(saved_claim_id)
    ...
    BenefitsIntake::SubmissionStatusJob.new.perform(EmploymentQuestionnaires::FORM_ID)

...
module Veteran
  class VsoReloader < BaseReloader
    ...
    def find_or_create_attorneys(attorney)
      ...
      rep.user_types << 'attorney' unless rep.user_types.contains?('attorney')
      rep.save
    end

    def find_or_create_claim_agents(claim_agent)
      rep = find_or_initialize(claim_agent)
      rep.user_types << 'claim_agents' unless rep.user_types.contains?('claim_agents')
      rep.save
    end
    ...
    def find_or_create_vso(vso)
      ...
      rep.user_types << 'veteran_service_officer' unless rep.user_types.contains?('veteran_service_officer')
      rep.save
    end
  end
end

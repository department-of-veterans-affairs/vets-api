  ...
  class AppointmentStatusNotificationCallback
    ...
    def self.extract_metadata(notification)
      ...
      if metadata.empty? || metadata['user_uuid'].blank? || metadata['appointment_id_last4'].blank?
        ...
        StatsD.increment("#{STATSD_KEY}.missing_metadata", tags: ['appointment_type:community_care'])
      end
    ...
    def self.handle_notification_status(notification, base_data)
      ...
      when 'delivered'
        StatsD.increment("#{STATSD_KEY}.success", tags: ['appointment_type:community_care'])
    ...
    def self.handle_failure(notification, base_data)
              StatsD.increment("#{STATSD_KEY}.failure", tags: ['appointment_type:community_care'])
    ...
    def self.handle_unknown_status(notification, base_data)
              StatsD.increment("#{STATSD_KEY}.unknown_status", tags: ['appointment_type:community_care'])

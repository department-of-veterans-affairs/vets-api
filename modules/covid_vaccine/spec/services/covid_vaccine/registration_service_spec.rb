...
describe CovidVaccine::RegistrationService do
  ...
  let (:mvi_profile_response) { MPI::Responses::FindProfileResponse.new(
      status: MPI::Responses::FindProfileResponse::RESPONSE_STATUS[:ok],
      profile: mvi_profile
    ) }
  let (:mvi_profile_not_found) { MPI::Responses::FindProfileResponse.new(
  ...
  describe "#register" do
    context 'unauthenticated' do
      ...
      it "coerces input to vetext format" do
        expect_any_instance_of(Vetext::V0::VaccineRegistryService).to receive(:put_vaccine_registry)
        ...
        response = subject.register(form_data)
      end
      ...
      it "saves a submission record" do
        ...
        expect_any_instance_of(MPI::Service).to receive(:find_profile)
          .and_return(mvi_profile_response)
        response = subject.register(form_data)
        expect(CovidVaccine::RegistrationSubmission.find_by(sid: sid)).to be_truthy
      end
      ...
      context 'with insufficient traits' do
        it "omits MPI query" do
          ...
          response = subject.register(sparse_form_data)
        end
      end 
    end
    ...
    context 'authenticated LOA3' do
      ...
      it "uses traits from proofed user" do
        expect_any_instance_of(Vetext::V0::VaccineRegistryService).to receive(:put_vaccine_registry)
          ...
          .and_return({sid: SecureRandom.uuid})
        response = subject.register_loa3_user(form_data, user)
      end

...
RSpec.describe 'Veteran Verification Documentation Endpoints', type: :request do
  ...
  describe '#get /docs/v0/veteran_verification' do
    ...
    it 'uses yaml that exists' do
      ...
      expect(disability_yaml['paths']['/disability_rating'].keys.size).to be_positive
      expect(disability_yaml['components']['schemas'].keys.size).to be_positive

      expect(status_yaml['paths']['/status'].keys.size).to be_positive
      expect(status_yaml['components']['schemas'].keys.size).to be_positive

      expect(service_history_yaml['paths']['/service_history'].keys.size).to be_positive
      expect(service_history_yaml['components']['schemas'].keys.size).to be_positive
    end
  end
end

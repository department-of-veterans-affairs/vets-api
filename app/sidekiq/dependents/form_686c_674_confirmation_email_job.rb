...
class Dependents::Form686c674ConfirmationEmailJob
  include Sidekiq::Job

  FORM_ID = '686C-674'
  FORM_ID_674 = '21-674'
  STATSD_KEY_PREFIX = 'api.dependents.form_686c_674_confirmation_email_job'
  ZSF_DD_TAG_FUNCTION = '686c_674_confirmation_email_queuing'

  sidekiq_options retry: 16

  sidekiq_retries_exhausted do |msg, ex|
    Rails.logger.error('Form686c674ConfirmationEmailJob exhausted all retries',
                       {
  ...
  def perform(claim_id, email, template_id, personalisation)
    ...
    Rails.logger.warn('Form686c674ConfirmationEmailJob failed, retrying send...', { claim_id:, error: e.message })
  end

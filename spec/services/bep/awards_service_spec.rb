...
RSpec.describe BEP::AwardsService do
  let(:user) { create(:evss_user, :loa3) }

  describe '#get_awards' do
    context 'with a valid participant id' do
      it 'returns the reward amounts' do
        VCR.use_cassette('bep/awards_service/get_awards') do
          service = BEP::AwardsService.new(user)
    ...
    context 'BEP does not return a response' do

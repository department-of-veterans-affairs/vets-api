...
module DependentsBenefits
  ...
  class DependentSubmissionJob
    ...
    def perform(claim_id, proc_id = nil)
      ...
      return if parent_claim_group_failed?
    ...
    def handle_job_success
      ActiveRecord::Base.transaction do
        parent_claim_group.with_lock do
          ...
          mark_claim_group_succeeded if all_siblings_succeeded?

          if all_child_groups_succeeded? && !parent_claim_group_completed?
    ...
    def all_child_groups_succeeded?
      SavedClaimGroup.where(
        claim_group_guid: parent_claim_group.claim_group_guid,
        parent_claim_id: parent_claim_group.parent_claim_id
      ).where.not(saved_claim_id: parent_claim_group.parent_claim_id)
                     .all?(&:succeeded?)
    end
    ...
    def handle_permanent_failure(claim_id, exception)
      ...
      ActiveRecord::Base.transaction do
        parent_claim_group.with_lock do
          ...
          mark_claim_group_failed

          unless parent_claim_group_completed?
    ...
    def parent_claim_group_failed?
      parent_claim_group&.failed?
    end

    # Check if parent claim group already completed (failed or succeeded)
    def parent_claim_group_completed?
      parent_claim_group&.completed?
    end
    ...
    def all_siblings_succeeded?
      SavedClaimGroup.where(
        claim_group_guid: parent_claim_group.claim_group_guid,
        parent_claim_id: parent_claim_group.parent_claim_id
      ).where.not(saved_claim_id: [claim_id, parent_claim_group.parent_claim_id]).all?(&:succeeded?)
    end

    def mark_claim_group_succeeded
      claim_group&.update!(status: SavedClaimGroup::SUCCESS)
    end

    def mark_claim_group_failed
      claim_group&.update!(status: SavedClaimGroup::FAILURE)
    end

    def mark_parent_claim_group_succeeded
      parent_claim_group&.update!(status: SavedClaimGroup::SUCCESS)
    end
    ...
    def parent_claim_group
      SavedClaimGroup.find_by(parent_claim_id: claim_group&.parent_claim_id,
                              saved_claim_id: claim_group&.parent_claim_id,

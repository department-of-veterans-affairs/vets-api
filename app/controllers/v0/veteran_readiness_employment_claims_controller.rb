# frozen_string_literal: true

module V0
  class VeteranReadinessEmploymentClaimsController < ApplicationController
    def create
      # load_user Might need this, not sure
      claim = SavedClaim::VeteranReadinessEmploymentClaim.new(form: filtered_params.to_json)
      claim.add_claimant_info(current_user) if current_user

      # unless claim.save
      #   StatsD.increment("#{stats_key}.failure")
      #   Raven.tags_context(team: 'vfs-ebenefits') # tag sentry logs with team name
      #   raise Common::Exceptions::ValidationErrors, claim
      # end
      #
      # Rails.logger.info "ClaimID=#{claim.confirmation_number} Form=#{claim.class::FORM}"
      claim.send_to_vre
    end

    private

    def filtered_params
      params.require(:veteran_readiness_employment_claim).permit(
        :use_eva,
        :use_telecounseling,
        :years_of_education,
        :is_moving,
        :main_phone,
        :cell_phone,
        :email,
        :privacy_agreement_accepted,
        veteran_address: {},
        new_address: {},
        appointment_time_preferences: {}
      )
    end
  end
end

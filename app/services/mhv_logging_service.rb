...
class MHVLoggingService
  def self.login(current_user)
    ...
    return false if current_user.mhv_correlation_ids.none? || current_user.mhv_last_signed_in
    # Otherwise send the login audit trail
    MHVLogging::Client.new(session: { user_id: current_user.mhv_correlation_ids.first })
                      .authenticate
                      .auditlogin
  ...
  def self.logout(current_user)
    ...
    return false unless current_user.mhv_correlation_ids.none? || current_user.mhv_last_signed_in

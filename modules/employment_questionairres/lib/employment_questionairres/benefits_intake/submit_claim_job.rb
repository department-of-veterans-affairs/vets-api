...
module EmploymentQuestionairres
  module BenefitsIntake
    ...
    class SubmitClaimJob
      ...
      # Error if "Unable to find EmploymentQuestionairres::SavedClaim"
      class EmploymentQuestionairresBenefitIntakeError < StandardError; end

      # retry for  2d 1h 47m 12s
      # https://github.com/sidekiq/sidekiq/wiki/Error-Handling
      sidekiq_options retry: 16, queue: 'low'
      sidekiq_retries_exhausted do |msg|
        ia_monitor = EmploymentQuestionairres::Monitor.new
        begin
          claim = EmploymentQuestionairres::SavedClaim.find(msg['args'].first)
        rescue
          claim = nil
        end
      ...
      def init(saved_claim_id, user_account_uuid)
        ...
        @claim = EmploymentQuestionairres::SavedClaim.find(saved_claim_id)
        unless @claim
          raise EmploymentQuestionairresBenefitIntakeError,
                "Unable to find EmploymentQuestionairres::SavedClaim #{saved_claim_id}"
        end
      ...
      def upload_document
        ...
        raise EmploymentQuestionairresBenefitIntakeError, response.to_s unless response.success?
      end
      ...
      def send_submitted_email
        EmploymentQuestionairres::NotificationEmail.new(@claim.id).deliver(:submitted)
      rescue => e

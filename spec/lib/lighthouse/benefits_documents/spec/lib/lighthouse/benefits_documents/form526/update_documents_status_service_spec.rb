...
RSpec.describe BenefitsDocuments::UpdateDocumentsStatusService do
  ...
  describe '#call' do
    let(:pending_document_upload) { create(:lighthouse_document_upload, document_type: 'Veteran Upload') }
    let(:uploads) { EvidenceSubmission.where(id: pending_document_upload.id) }
    ...
    context 'when a Lighthouse526DocumentUpload is still in progress at Lighthouse' do
      ...
      context 'when updating multiple records' do
        let!(:first_pending_polling_document) { create(:lighthouse_document_upload, aasm_state: 'pending') }
        let!(:second_pending_polling_document) { create(:lighthouse_document_upload, aasm_state: 'pending') }
    ...
    describe 'document type in statsd metrics' do
      ...
      shared_examples 'correct statsd metric' do |document_type, metrics_key|
        let(:document_upload) { create(:lighthouse_document_upload, document_type:) }

...
RSpec.describe LegacyFilterQueryRewriteMiddleware, type: :request do
  ...
  context 'when cookies are present in the env' do
    let(:query_string) { 'filter[[disp_status][eq]]=Active,Expired&other=123' }
    let(:cookies) { 'session_id=abc123; user_token=xyz789' }
    let(:env) { { 'QUERY_STRING' => query_string, 'HTTP_COOKIE' => cookies } }

    it 'preserves cookies in the environment' do
      _status, new_env, _body = middleware.call(env)
      expect(new_env['HTTP_COOKIE']).to eq(cookies)
    end
  end
end

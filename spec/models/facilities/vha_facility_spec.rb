  ...
  RSpec.describe VHAFacility do
    ...
    describe 'pull_source_data' do
      it 'pulls data from a GIS endpoint' do
        VCR.use_cassette('facilities/va/vha_facilities_limit_results') do
          ...
          expect(list.all? { |item| item.is_a?(VHAFacility) }).to be true
        end
      end
      ...
      context 'with single facility' do
        ...
        context 'with services' do
          ...
          let(:sat_client_stub) { instance_double('AccessSatisfactionClient') }
          let(:wait_client_stub) { instance_double('AccessWaitTimeClient') }

          before do
            allow(AccessSatisfactionClient).to receive(:new) { sat_client_stub }
            allow(AccessWaitTimeClient).to receive(:new) { wait_client_stub }
            ...
            AccessDataDownload.new.perform
            DentalServiceReloadJob.new.perform
          end
    ...
    describe 'with_services' do
      it 'returns a list of facilities that provide the selected services' do
        ...
        result = VHAFacility.with_services(['UrgentCare'])

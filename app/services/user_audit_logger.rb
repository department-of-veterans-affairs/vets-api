...
class UserAuditLogger
  ...
  def self.log(user_action_event_id:, acting_user:, subject_user:,
               status: :initial, ip_address: nil, user_agent: nil)
    if subject_user.user_verification.nil?
      raise MissingSubjectVerificationError, 'Subject user must have a verification'
    end

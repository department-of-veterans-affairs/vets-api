  ...
  describe '#build_geocodable_address' do
    context 'with full address available' do
      ...
      it 'returns the full address string' do
        expect(representative.send(:build_geocodable_address)).to eq('123 Main St, Springfield, IL, 62701')
      end
    end
    ...
    context 'with only city and state available' do
      ...
      it 'returns city and state' do
        expect(representative.send(:build_geocodable_address)).to eq('Springfield, IL')
      end
    end
    ...
    context 'with only zip code available' do
      ...
      it 'returns just the zip code' do
        expect(representative.send(:build_geocodable_address)).to eq('62701')
      end
    end
    ...
    context 'with no address data available' do
      ...
      it 'returns nil' do
        expect(representative.send(:build_geocodable_address)).to be_nil
      end
    end
    ...
    context 'with partial address (missing zip)' do
      ...
      it 'returns address without zip' do
        expect(representative.send(:build_geocodable_address)).to eq('123 Main St, Springfield, IL')
      end
    end
  end
end

...
RSpec.describe BGS::DependentService do
  ...
  context 'claims evidence enabled' do
    ...
    it 'submits evidence pdf via claims evidence uploader' do
      ...
      expect(monitor).to receive(:track_event).with('info', 'BGS::DependentService#submit_pdf_job called to begin ClaimsEvidenceApi::Uploader',
      ...
      expect(monitor).to receive(:track_event).with('info', "BGS::DependentService claims evidence upload of 686C-674 claim_id #{claim.id}",
                                                    "#{stats_key}.claims_evidence.upload", tags: ["form_id:686C-674"])
      expect(monitor).to receive(:track_event).with('info', "BGS::DependentService claims evidence upload of 21-674 claim_id #{claim.id}",

...
RSpec.describe 'Mobile::V0::Appointments::VAOSV2', type: :request do
  ...
  shared_examples 'appointment tests' do |parallel_enabled|
    before do
      allow(Flipper).to receive(:enabled?).with(:appointments_parallel_processing).and_return(parallel_enabled)
    end
    ...
    context 'with VAOS' do
      ...
      describe 'GET /mobile/v0/appointments' do
        ...
        context 'travel pay claims' do
          ...
          it 'uses parallel processing when feature toggle is enabled' do
            # Enable both required feature flags
            allow(Flipper).to receive(:enabled?).with(:travel_pay_view_claim_details,
                                                      instance_of(User)).and_return(true)
            allow(Flipper).to receive(:enabled?).with(:appointments_parallel_processing,
                                                      instance_of(User)).and_return(true)

            # Spy on the parallel method to verify it's called
            expect_any_instance_of(VAOS::V2::AppointmentsService)
              .to receive(:get_appointments_with_parallel_travel_pay).and_call_original
            ...
            expect(response).to have_http_status(:ok)
            # Verify travel pay claims are still properly attached despite parallel processing
            expect(response.parsed_body['meta']['travelPayEligibleCount']).to be_present
          end

          it 'uses standard processing when parallel feature toggle is disabled' do
            # Enable travel pay but disable parallel processing
            allow(Flipper).to receive(:enabled?).with(:travel_pay_view_claim_details,
                                                      instance_of(User)).and_return(true)
            allow(Flipper).to receive(:enabled?).with(:appointments_parallel_processing,
                                                      instance_of(User)).and_return(false)

            # Verify the parallel method is NOT called
            expect_any_instance_of(VAOS::V2::AppointmentsService)
              .not_to receive(:get_appointments_with_parallel_travel_pay)
            ...
            expect(response).to have_http_status(:ok)
            # Verify travel pay claims are still properly attached with standard processing
            expect(response.parsed_body['meta']['travelPayEligibleCount']).to be_present
          end
        end
  ...
  # Run tests with parallel processing disabled (default behavior)
  describe 'with parallel processing disabled' do
    include_examples 'appointment tests', false
  end

  # Run tests with parallel processing enabled
  describe 'with parallel processing enabled' do
    include_examples 'appointment tests', true
  end
end

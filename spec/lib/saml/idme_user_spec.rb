...
RSpec.describe SAML::User do
  describe 'ID.me' do
    ...
    context 'LOA3 user' do
      ...
      it 'returns loa current 1 if KeyError in LOA_MAPPING lookup for loa_current' do
        stub_const('SAML::UserAttributes::IdMe::LOA_MAPPING', {})
    ...
    context 'MHV LOA3 user' do
      let(:saml_response) { SAML::ResponseBuilder.saml_response_from_attributes('myhealthevet_loa3', saml_attributes) }
      let(:saml_attributes) do
        OneLogin::RubySaml::Attributes.new(
          'uuid'               => ['1234abcd'],
          'email'              => ['john.adams@whitehouse.gov'],

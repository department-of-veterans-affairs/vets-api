...
RSpec.describe V0::VirtualAgentClaimLettersController do
  ...
  describe '#index' do
    ...
    it 'lists only documents of the doctypes specified by the feature flags' do
      ...
      allow(Flipper).to receive(:enabled?).with(:cst_include_ddl_5103_letters, nil).and_return(true)
      allow(Flipper).to receive(:enabled?).with(:cst_include_ddl_boa_letters, nil).and_return(false)
      get(:index)
      letters = JSON.parse(response.body)

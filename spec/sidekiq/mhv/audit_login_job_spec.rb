...
RSpec.describe MHV::AuditLoginJob do
  ...
  let(:user_verification) { instance_double(UserVerification, user_account: user_account, user_uuid: user_uuid) }
  ...
  describe 'perform' do
    it 'audits an MHV login for a user with nil mhv_last_signed_in' do
    ...
    context 'with multiple users having the same MHV correlation ID' do
      ...
      let(:delegate_user) { instance_double(User, uuid: delegate_uuid, mhv_correlation_id: mhv_correlation_id, mhv_last_signed_in: nil) }
    ...
    context 'with user_account_id provided' do
      let(:user_with_account) { instance_double(User, uuid: user_uuid, mhv_correlation_id: mhv_correlation_id, mhv_last_signed_in: nil) }
      ...
      let(:other_user) { instance_double(User, uuid: 'other-uuid', mhv_correlation_id: mhv_correlation_id, mhv_last_signed_in: nil) }
      ...
      it 'updates only the user associated with the account' do
        described_class.new.perform(mhv_correlation_id, nil, 1)

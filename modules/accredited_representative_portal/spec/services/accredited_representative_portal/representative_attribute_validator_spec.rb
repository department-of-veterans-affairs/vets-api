...
RSpec.describe AccreditedRepresentativePortal::RepresentativeAttributeValidator do
  describe '#perform' do
    subject { AccreditedRepresentativePortal::RepresentativeAttributeValidator.new(user_attributes:).perform }
    ...
    context 'when credential is verified' do
      ...
      let(:add_person_response) { nil }
      let(:find_profile_response) { nil }
      let(:update_profile_response) { nil }
      ...
      shared_examples 'credential mpi verification' do
        ...
        context 'and mpi record does not exist for user' do
          ...
          before do
            allow_any_instance_of(AccreditedRepresentativePortal::RepresentativeAttributeValidator)
              .to receive(:mpi_record_exists?).and_return(false)
          end
      ...
      context 'and authentication is with idme' do
        ...
        context 'and credential is not missing any required attributes' do
          context 'and the credential matches a Representative record' do
            it_behaves_like 'credential mpi verification'
          end

          context 'and the credential does not match a Representative record' do
            let(:representative_attributes) { nil }

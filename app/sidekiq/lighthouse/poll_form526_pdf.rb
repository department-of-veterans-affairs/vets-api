  ...
  class PollForm526Pdf
    ...
    def update_job_status_based_on_age(form_job_status, submission)
      if submission.created_at.between?(DateTime.now - 4.days, DateTime.now - 1.day)
        PollForm526PdfStatus.update_job_status(
          form_job_status:,
          message: "Poll for form 526 PDF: Submission creation date is over 1 day old for submission_id #{submission.id}",
          error_class: 'PollForm526PdfError',
          error_message: "Poll for form 526 PDF: Submission creation date is over 1 day old for submission_id #{submission.id}",

...
module MebApi
  module V0
    class EducationBenefitsController < MebApi::V0::BaseController
      ...
      def claim_status
        claimant_response = claimant_service.get_claimant_info
        claimant_id = claimant_response.body['claimant_id']
        claim_status_response = claim_status_service.get_claim_status(params, claimant_id)

        response = claimant_response.status_code == 201 ? claim_status_response : claimant_response
        serializer = claimant_response.status_code == 201 ? ClaimStatusSerializer : ClaimantSerializer

...
RSpec.describe Vye::V1::VerificationsController, type: :controller do
  ...
  describe 'eval_case4' do
    ...
    describe 'happy path(s)' do
      context 'when abd <= ldpm <= dlc < aed <= rd' do
        ...
        it 'creates a case4 pending verification' do
          ...
          check_expectations_for(pv, date_last_certified, aed_minus1, aed_minus1, 'case4')
        end
      end

      context 'when abd < ldpm < dlc < aed < rd' do
        # Just to make sure no goofiness around act begin and act end,
        # make the date differences noticeable

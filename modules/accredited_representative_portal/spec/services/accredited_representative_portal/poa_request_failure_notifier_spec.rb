  ...
  RSpec.describe PoaRequestFailureNotifier do
    describe '#call' do
      ...
      before do
        ...
        allow(Flipper).to receive(:enabled?).with(:ar_poa_request_failure_notification_email_for_claimant)
                                            .and_return(claimant_enabled)
        allow(Flipper).to receive(:enabled?).with(:ar_poa_request_failure_notification_email_for_rep)

  ...
  describe 'on complete' do
    ...
    context 'with a 403 error' do
      it 'raises VAOS_403 BackendServiceException' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_403) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
    ...
    context 'with a 404 error' do
      it 'raises VAOS_404 BackendServiceException' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_404) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
    ...
    context 'with a 500..510 error' do
      it 'raises VAOS_502 error' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_500) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
    ...
    context 'with a 500 vamf status on cancelled appointments' do
      it 'raises VAOS_400 error' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_500a) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
          expect(e.key).to equal('VAOS_400')
    ...
    context 'with all other errors' do
      it 'raises a VA900 error' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_other) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
    ...
    context 'with error' do
      it 'parses error message' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_with_error) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
          expect(e.key).to equal('VAOS_400')
    ...
    context 'with errors' do
      it 'parses error message' do
        err = VAOS::Middleware::Response::Errors.new
        expect { err.on_complete(env_with_errors) }.to raise_error(Common::Exceptions::BackendServiceException) { |e|
          expect(e.key).to equal('VAOS_400')

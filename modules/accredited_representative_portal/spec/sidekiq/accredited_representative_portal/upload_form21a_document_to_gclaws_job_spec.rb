...
RSpec.describe AccreditedRepresentativePortal::UploadForm21aDocumentToGCLAWSJob, type: :job do
  ...
  let(:form21a_attachment) do
    create(:form_attachment, guid: form21a_attachment_guid, type: 'AccreditedRepresentativePortal::Form21aAttachment')
  end
  ...
  describe '#perform' do
    ...
    context 'when file retrieval from S3 fails with a non-retryable error' do
      before do
        form21a_attachment

        allow_any_instance_of(FormAttachment)
          .to receive(:get_file)
          .and_raise(StandardError, 'S3 connection failed')
      end

      it 'logs a non-retryable error and does not raise' do
        expect(Rails.logger).to receive(:info).with(/Starting upload/)

        # Make logger spy-able for error
        allow(Rails.logger).to receive(:error).and_call_original

        expect { perform_job }.not_to raise_error

        # Assert that at least one error log contains our message
        expect(Rails.logger).to have_received(:error)
          .with(/Non-retryable error retrieving file from S3/)
      end
    end

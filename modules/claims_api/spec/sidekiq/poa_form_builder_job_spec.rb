...
RSpec.describe ClaimsApi::PoaFormBuilderJob, type: :job do
  ...
  let(:power_of_attorney_v1) { create(:power_of_attorney, :with_full_headers) }
  let(:power_of_attorney_v2) { create(:power_of_attorney, :with_full_headers) }
  let(:poa_code) { 'ABC' }
  ...
  before do
    ...
    power_of_attorney_v1.form_data = {
      recordConcent: true,
    ...
    power_of_attorney_v1.save

    power_of_attorney_v2.form_data = {
      recordConcent: true,
      consentAddressChange: true,
      consentLimits: ['DRUG ABUSE', 'SICKLE CELL'],
      signatures: {
  ...
  describe 'generating the filled and signed pdf' do
    context 'when representative is an individual' do
      ...
      it 'generates the pdf to match example for v1' do
        ...
        subject.new.perform(power_of_attorney_v1.id)
      end

      it 'generates the pdf to match example for v2' do
        allow_any_instance_of(BGS::PersonWebService).to receive(:find_by_ssn).and_return({ file_nbr: '123456789' })
        expect(ClaimsApi::PoaPdfConstructor::Individual).to receive(:new).and_call_original
        expect_any_instance_of(ClaimsApi::PoaPdfConstructor::Individual).to receive(:construct).and_call_original
        subject.new.perform(power_of_attorney_v2.id)
      end
    ...
    context 'when signature has prefix' do
      before do
        ...
        Veteran::Service::Organization.create(poa: 'ABC', name: 'Some org')
        power_of_attorney_v1.update(form_data: power_of_attorney_v1.form_data.deep_merge(
          {
            signatures: {
      ...
      it 'sets the status and store the error' do
        ...
        subject.new.perform(power_of_attorney_v1.id)
        power_of_attorney_v1.reload
        expect(power_of_attorney_v1.status).to eq(ClaimsApi::PowerOfAttorney::ERRORED)
        expect(power_of_attorney_v1.signature_errors).not_to be_empty
      end
    end
  end
end

  ...
  def pdfs_fields_match?(pdf_1_path, pdf_2_path)
    ...
    fields[0].each do |field_1|
      ...
      if matching_field.nil?
        ...
        mismatches << "Field '#{field_1[:name]}' mismatch: temp PDF = '#{field_1[:value]}', physical PDF = '#{matching_field[:value]}'"
      end
    end
    ...
    unless mismatches.empty?
      puts "PDF Field Mismatches: #{error_number}"
      mismatches.each { |m| puts "#{m} - #{error_number += 1}" }
    end

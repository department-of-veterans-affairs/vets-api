...
module CheckIn
  ...
  class TravelClaimNotificationJob < TravelClaimBaseJob
    ...
    REQUIRED_FIELDS = %i[mobile_phone template_id appointment_date claim_number].freeze
    ...
    def log_missing_fields(missing_data)
      logger.info({ message: "SMS notification skipped: missing #{missing_data.join(', ')}" })
      StatsD.increment(Constants::STATSD_NOTIFY_ERROR)
    end
    ...
    def parse_appointment_date(date_string)
      ...
      logger.info({ message: 'SMS notification skipped: invalid appointment date format' })
      StatsD.increment(Constants::STATSD_NOTIFY_ERROR)
      nil
    end

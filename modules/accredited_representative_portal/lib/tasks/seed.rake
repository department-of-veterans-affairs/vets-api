...
module AccreditedRepresentativePortal
  module Seed
    class << self
      def run
        # With representative data derived from [here](https://github.com/department-of-veterans-affairs/va.gov-team-sensitive/blob/44b9fdb497587a46837b65f55827e16e1fde6547/products/accredited-representation-management/accredited-entities-test-data.md?plain=1#L1)
        upsert_organizations
        accreditations = upsert_representatives
        upsert_claims_agents
        upsert_attorneys

        upsert_poa_requests(accreditations)
      end
      ...
      def upsert_organizations
        organizations =
          ORGANIZATIONS.map do |organization|
            FactoryBot
              .build(:accredited_organization, **organization)
              .attributes
              .compact!
          end

        AccreditedOrganization.upsert_all(
          organizations,
      ...
      def upsert_representatives
        accreditations = []

        representatives =
          REPRESENTATIVES.map do |representative|
            representative.delete(:organization_ids).each do |organization_id|
              accreditations << {
            ...
            FactoryBot
              .build(:accredited_individual, :representative, **representative)
              .attributes
              .compact!
          end

        AccreditedIndividual.upsert_all(
          representatives,
          record_timestamps: true
        )

        Accreditation.upsert_all(
          accreditations,
          unique_by: [:accredited_individual_id, :accredited_organization_id],
          record_timestamps: true
        )

        Accreditation.where(<<~SQL, *accreditations.map(&:values).flatten).to_a
          (accredited_individual_id, accredited_organization_id) IN
          (#{['(?,?)'] * accreditations.size * ','})
        SQL
      end
      ...
      def upsert_claims_agents
        claims_agents =
          CLAIMS_AGENTS.map do |claims_agent|
            FactoryBot
              .build(:accredited_individual, :claims_agent, **claims_agent)
              .attributes
              .compact!
          end
      ...
      def upsert_attorneys
        attorneys =
          ATTORNEYS.map do |attorney|
            FactoryBot
              .build(:accredited_individual, :attorney, **attorney)
              .attributes
              .compact!
          end
      ...
      # Make one claimant per accreditation. The claimant then gets a permutation
      ...
      def upsert_poa_requests(accreditations)
        resolution_types = [:expiration, :declination, :acceptance]
        resolution_histories_cycle = resolution_types.permutation.cycle
        accreditations_cycle = accreditations.cycle

        claimants =
          CLAIMANT_IDS.map do |id|
            FactoryBot
              .build(:user_account, id:)
              .attributes
              .compact!
          end

        UserAccount.upsert_all(
          claimants,
          record_timestamps: true
        )

        claimants = UserAccount.where(id: CLAIMANT_IDS).to_a
        ids = POA_REQUEST_IDS.to_enum
        claimant_power_of_attorney_forms = {}

        claimants.each do |claimant|
          power_of_attorney_form = FactoryBot.build(:dynamic_power_of_attorney_form)
          claimant_power_of_attorney_forms[claimant.id] = power_of_attorney_form

          resolution_histories_cycle.next.each do |resolution_type|
            # Still need to cycle even if we're going to skip, so that everything
            # stays the same.
            accreditation = accreditations_cycle.next
            created_at = TIME_TRAVELER.next

            id = ids.next
            next if PowerOfAttorneyRequest.exists?(id)

            power_of_attorney_request =
              FactoryBot.create(
                :power_of_attorney_request,
                id:, created_at:, claimant_id: claimant.id,
                power_of_attorney_holder: accreditation.accredited_organization,
                accredited_individual: accreditation.accredited_individual,
                power_of_attorney_form: power_of_attorney_form.dup
              )

            FactoryBot.create(
              :power_of_attorney_request_resolution, resolution_type,
        ...
        # Now make one unresolved POA request per individual that is more 
        # recent than any of the claimant's resolved ones. The math on all the
        # time traveling just works out for this number of accreditations (37).
        accredited_individuals = accreditations.map(&:accredited_individual).uniq
        accredited_individuals.each_with_index do |accredited_individual, index|
          # Still need to cycle even if we're going to skip, so that everything
          # stays the same.
          created_at = RECENT_TIME_TRAVELER.next

          id = ids.next
          next if PowerOfAttorneyRequest.exists?(id)

          claimant = claimants[index]
          accreditation = accredited_individual.accreditations.first
          power_of_attorney_form = claimant_power_of_attorney_forms[claimant.id]
      ...
      CLAIMANT_IDS = %w[
        7de8ce1e-af1c-4c5d-8caf-4002593dac69 3e6f543c-1d24-456e-be14-49e0a0d6eeb1 1e6f8e10-3515-498c-b241-00f110ead8e3
        fe09af3c-d408-40b2-88e4-b3d78d05a88b 8da29bdb-29e3-4b8a-a462-e640d34fdc76 dfe94d0f-4c19-4105-9bf3-eec850b651ac
        6f1c7f54-46ce-4336-9c4e-103263457cf5 77d8f6f9-326d-4304-a61c-f74f68f2d533 04f6698b-68c8-40ff-ab82-07899e09eaa8
        7f26a96a-713c-4780-befe-339b30c98ba9 9c86e2f5-a4de-4b91-8822-66ccf560db7b 3a764877-02b3-4b8d-b5e7-66204ef67cc3
        ca2ec8b1-0733-42eb-9e10-6a5ff2c077e2 09d1b85a-8e1b-48b3-b16c-b6b6d1fc8521 6febeedd-a134-4317-85d1-3bad378de80e
        1b879de8-2e8e-4a3f-8f7a-72edcb97ce30 a763d926-33d0-47ad-87d2-cfad63b74132 9529f8f9-9d28-4a8e-bef6-d9b47d6adf08
        7c1236f0-ba8c-4d0c-be76-7dab86150757 e7930f8c-23be-4b97-b4b1-4c006f034a9c 40407704-aee4-4c66-998f-1ab9f8d2aa3f
        eda90664-4002-44b0-9028-e99156a0f2f1 c93694ce-d64d-4eab-b0a9-4e8da9d6cb0e 8bea4d28-da9f-43eb-836f-6d24b4bb5061
        ce02c3d8-412f-43af-9711-05771224f09d 40ff03e4-19b6-4f70-bdb7-8d6ebe14b11b 427f8301-3c37-4311-a318-b71be231de38
        9c1eef4b-6f43-42c6-8361-9d197ece3d37 dbce479d-88cd-4467-a771-4fab184425f3 0ec8246c-259e-41d5-a6fc-d4e12a7f3bad
        4cdad2aa-87b9-4879-96fe-3effd6a13d83 e61c1158-7d2b-43f3-8a9f-046ffdf1df37 80a37238-7f0d-426f-ba14-e76e089fd24b
        fcfd3511-2427-4ebd-bc73-8f88e12e6339 8c517f83-8cc1-456e-85c0-bfbeb0f11c65 bf7a3db5-e103-4e87-b009-ca7762169ec8
        a88f2205-949f-40c4-a925-76a053ddf1b8
      ].freeze
      ...
      TIME_TRAVELER = Enumerator.new do |yielder|
        time = 30.days.ago
        loop do
          yielder << time + 0.days
      ...
      RECENT_TIME_TRAVELER = Enumerator.new do |yielder|
        time = 10.days.ago
        loop do
          yielder << time
          time += 6.hours

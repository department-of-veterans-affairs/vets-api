...
MyHealth::Engine.routes.draw do
  ...
  namespace :v1 do
    ...
    resource :health_records, only: [:create], defaults: { format: :json } do
      ...
      post 'sharing/optin', to: 'vhie_sharing#optin', on: :collection
      post 'sharing/optout', to: 'vhie_sharing#optout', on: :collection
      get 'sharing/status', to: 'vhie_sharing#status', on: :collection
    end
  end

...
RSpec.describe EmailVerificationRateLimited, type: :controller do
  ...
  before do
    ...
    allow(redis_client).to receive(:get).and_return('0')
    allow(redis_client).to receive(:ttl).and_return(0)
  ...
  describe '#email_verification_rate_limit_exceeded?' do
    context 'when neither limit is exceeded' do
      before do
        allow(controller).to receive(:verification_period_count).and_return(0)
        allow(controller).to receive(:verification_daily_count).and_return(3)
      end
    ...
    context 'when period limit is exceeded' do
      before do
        allow(controller).to receive(:verification_period_count).and_return(1)
        allow(controller).to receive(:verification_daily_count).and_return(3)
      end
    ...
    context 'when daily limit is exceeded' do
      before do
        allow(controller).to receive(:verification_period_count).and_return(0)
        allow(controller).to receive(:verification_daily_count).and_return(5)
      end
    ...
    context 'when both limits are exceeded' do
      before do
        allow(controller).to receive(:verification_period_count).and_return(2)
        allow(controller).to receive(:verification_daily_count).and_return(6)
      end
  ...
  describe '#enforce_email_verification_rate_limit!' do
    ...
    context 'when rate limit exceeded' do
      before do
        allow(controller).to receive(:email_verification_rate_limit_exceeded?).and_return(true)
        ...
        allow(controller).to receive(:time_until_next_verification_allowed).and_return(240)
      ...
      context 'when response is not defined' do
        ...
        before do
          allow(controller_no_response).to receive(:verification_redis).and_return(redis_client)
          allow(controller_no_response).to receive(:email_verification_rate_limit_exceeded?).and_return(true)
          ...
          allow(controller_no_response).to receive(:time_until_next_verification_allowed).and_return(240)
          allow(controller_no_response).to receive(:build_verification_rate_limit_message)
            .and_return('Rate limit exceeded message')
          allow(controller_no_response).to receive(:log_email_verification_rate_limit_denial)
        end

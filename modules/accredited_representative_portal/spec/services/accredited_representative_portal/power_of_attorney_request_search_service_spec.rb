...
RSpec.describe AccreditedRepresentativePortal::PowerOfAttorneyRequestSearchService do
  describe('.call') do
    ...
    it 'searches based on name, dob and ssn' do
      first_name = poa_request.power_of_attorney_form.veteran_first_name
      last_name = poa_request.power_of_attorney_form.veteran_last_name
      dob = poa_request.power_of_attorney_form.veteran_dob
      ssn = poa_request.power_of_attorney_form.veteran_ssn

      expect(
        described_class.new(poa_requests, first_name, last_name, dob, ssn).call
      ).to eq [poa_request]
    end
  end
end

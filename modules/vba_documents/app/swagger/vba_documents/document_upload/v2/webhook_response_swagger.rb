...
module VBADocuments
  module DocumentUpload
    module V2
      class WebhookResponseSwagger
        include Swagger::Blocks
        EXAMPLE_PATH = VBADocuments::Engine.root.join('app', 'swagger', 'vba_documents', 'document_upload', 'v2', 'webhook_response_example.json')
        swagger_component do
          schema :WebhookResponse do
            key :type, :object
            key :in, :formData
            key :example, JSON.parse(File.read(EXAMPLE_PATH))
            property :subscriptions do
              key :description, 'An object that must be passed for notifications'
              key :example, JSON.parse(File.read(EXAMPLE_PATH))
              key :type, :array
              items do
                property :event do

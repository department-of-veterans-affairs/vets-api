...
RSpec.shared_examples 'paginated request from params with expected IDs' do |request_params, ids, mobile = nil|
  ...
  context request_params do
    before do
      get '/ask_va_api/v0/health_facilities', params:
    end
    ...
    it 'is expected to include pagination links' do
      ...
      prev_link = prev_page ? "http://www.example.com/ask_va_api/v0/health_facilities?#{prev_params}" : nil
      next_link = next_page ? "http://www.example.com/ask_va_api/v0/health_facilities?#{next_params}" : nil

      expect(parsed_body[:links]).to match(
        self: "http://www.example.com/ask_va_api/v0/health_facilities?#{params.merge({ page: current_page,
                                                                                       per_page: 10 }).to_query}",
        first: "http://www.example.com/ask_va_api/v0/health_facilities?#{params.merge({ per_page: 10 }).to_query}",
        prev: prev_link,
        next: next_link,
        last: "http://www.example.com/ask_va_api/v0/health_facilities?#{params.merge({ page: last_page,
                                                                                       per_page: 10 }).to_query}"
      )
    end
  end
...
RSpec.describe AskVAApi::V0::HealthFacilitiesController, type: :request, vcr: vcr_options do
  ...
  describe 'GET #index' do
    it 'returns 400 for invalid type parameter' do
      get '/ask_va_api/v0/health_facilities', params: { type: 'bogus' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 for query with services but no type' do
      get '/ask_va_api/v0/health_facilities', params: { services: 'EyeCare' }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 for health query with unknown service' do
      get '/ask_va_api/v0/health_facilities', params: { type: 'health', services: ['OilChange'] }
      expect(response).to have_http_status(:bad_request)
    end

    it 'returns 400 for benefits query with unknown service' do
      get '/ask_va_api/v0/health_facilities', params: { type: 'benefits', services: ['Haircut'] }
      expect(response).to have_http_status(:bad_request)
    end
    ...
    it "sends a 'lighthouse.facilities.request.faraday' notification to any subscribers listening" do
      ...
      expect do
        get '/ask_va_api/v0/health_facilities', params: { bbox: [-122.786758, 45.451913, -122.440689, 45.64] }

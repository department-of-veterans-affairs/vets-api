...
module DebtsApi
  class V0::FsrForm
    ...
    def initialize(form, file_number, user = nil)
      ...
      @original_form = form
      @streamlined_data = form[STREAMLINED_KEY]
      ...
      @grouped_vha_copays = @vha_copays.group_by { |copay| copay['station']['facilitYNum'] }
      @user_form = get_user_form
      @vba_form = build_vba_form
      @vha_forms = build_vha_forms # {form: facility_form, copays: copays}
    ...
    def get_user_form
      sanitize(@original_form.deep_dup)
    end
    ...
    def get_vha_copays
      @all_debts&.filter { |debt| debt['debtType'] == VHA_TYPE_KEY }
    end

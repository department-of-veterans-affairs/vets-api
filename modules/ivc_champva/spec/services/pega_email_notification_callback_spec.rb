...
require_relative '../../app/services/ivc_champva/pega_email_notification_callback'
require_relative '../../lib/ivc_champva/monitor'

RSpec.describe IvcChampva::PegaEmailNotificationCallback do
  ...
  before do
    ...
    allow(monitor).to receive(:track_pega_alert_email_sent)
    allow(monitor).to receive(:track_pega_alert_email_failed)
  ...
  describe '.call' do
    context 'when status is delivered' do
      ...
      it 'tracks pega alert email sent' do
        expect(monitor).to receive(:track_pega_alert_email_sent).with('10-10d')
        described_class.call(notification)
      end
    end
    ...
    context 'when status is permanent-failure' do
      ...
      it 'tracks pega alert email failed' do
        expect(monitor).to receive(:track_pega_alert_email_failed).with('10-10d', 'permanent-failure', 'test_reason')
        described_class.call(notification)
      end
    end

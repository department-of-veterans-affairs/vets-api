...
module Mobile
  module V0
    class DisabilityRating
      ...
      def self.rating_service(user)
        BGS::Services.new(external_uid: user.icn, external_key: user.email)
      end

      def self.for_user(user)
        response = rating_service(user).rating.find_rating_data(user.ssn)

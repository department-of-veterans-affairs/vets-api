...
class CreateArmTables < ActiveRecord::Migration[7.1]
  ...
  def change
    ...
    safety_assured do
      execute 'ALTER TABLE accredited_representatives ADD PRIMARY KEY (number);' # rubocop:disable Rails/ReversibleMigration
    end
    add_index :accredited_representatives, :full_name
    add_index :accredited_representatives, :location, using: :gist
    add_index :accredited_representatives, %i[number first_name last_name], unique: true, name: 'index_rep_group'
    ...
    safety_assured do
      execute 'ALTER TABLE accredited_organizations ADD PRIMARY KEY (poa);' # rubocop:disable Rails/ReversibleMigration
    end
    ...
    add_index :accredited_representative_types, %i[accredited_representative_number type], unique: true,
                                                                                           name: 'index_accredited_representative_types_on_id_and_type' # rubocop:disable Layout/LineLength
    add_foreign_key :accredited_representative_types, :accredited_representatives,
                    column: :accredited_representative_number, primary_key: :number, validate: false
    ...
    add_index :accredited_organization_accredited_representatives,
              %i[accredited_representative_number accredited_organization_poa], unique: true, name: 'index_organization_representatives_on_rep_and_org' # rubocop:disable Layout/LineLength
    ...
    add_foreign_key :accredited_organization_accredited_representatives, :accredited_organizations,
                    column: :accredited_organization_poa, primary_key: :poa, validate: false
  end
end

...
RSpec.describe Mobile::PaginationHelper, type: :model, aggregate_failures: true do
  describe '.paginate' do
    ...
    it 'returns the requested number of records' do
      ...
      resources, meta = described_class.paginate(list: list, validated_params: params)
    ...
    it 'adds any provided errors to the meta' do
      ...
      _resources, meta = described_class.paginate(list: list, validated_params: params, errors: error_message)
    ...
    it 'raises an error if the provided validated_params are not a contract object' do
      expect do
        described_class.paginate(list: list, validated_params: { page_size: 10, page_number: 1 })
      end.to raise_error(

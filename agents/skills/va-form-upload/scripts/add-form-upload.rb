...
FORM_ID_UPLOAD = "#{FORM_ID}-UPLOAD".freeze
...
modify_file('app/models/form_profile.rb', 'form_profile.rb - ALL_FORMS[:form_upload]') do |content|
  next if content.include?(FORM_ID_UPLOAD.to_s)
...
modify_file('app/models/persistent_attachments/va_form.rb',
            "va_form.rb - CONFIGS (max: #{options[:max_pages]}, min: #{options[:min_pages]})") do |content|
...
modify_file('modules/simple_forms_api/app/services/simple_forms_api/notification/form_upload_email.rb',
            'form_upload_email.rb - SUPPORTED_FORMS (VANotify)') do |content|
  next if content.include?(FORM_ID)
...
if options[:stamp]
  modify_file('modules/simple_forms_api/app/services/simple_forms_api/scanned_form_stamps.rb',
              'scanned_form_stamps.rb - FORMS_WITH_STAMPS') do |content|
    next if content.include?(FORM_ID)
  ...
  if options[:stamp_page] && options[:stamp_page] != 0
    modify_file('modules/simple_forms_api/app/services/simple_forms_api/scanned_form_stamps.rb',
                "scanned_form_stamps.rb - STAMP_PAGE_OVERRIDES (page #{options[:stamp_page]})") do |content|
      next if content.include?("'#{FORM_ID}' =>")

      content.sub!(/(?<=STAMP_PAGE_OVERRIDES = \{)(.*?)(\s*\}.freeze)/m) do
        "#{Regexp.last_match(1)},\n      '#{FORM_ID}' => #{options[:stamp_page]}\n#{Regexp.last_match(2)}"
      end
    end
  end
else

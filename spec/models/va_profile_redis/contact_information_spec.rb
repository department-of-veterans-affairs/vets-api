...
describe VAProfileRedis::ContactInformation do
  ...
  let(:person_response) do
    ...
    VAProfile::ContactInformation::V1::PersonResponse.from(raw_response)
  end
  ...
  [404, 400].each do |status|
    context "with a #{status} from get_person", :skip_vet360 do
      ...
      before do
        ...
        service = double
        allow(VAProfile::ContactInformation::V1::Service).to receive(:new).with(user).and_return(service)
        expect(service).to receive(:get_person).public_send(
          get_person_calls
        ).and_return(
          VAProfile::ContactInformation::V1::PersonResponse.new(status, person: nil)
        )
      end
  ...
  describe '#response' do
    context 'when the cache is empty' do
      it 'caches and return the response', :aggregate_failures do
        ...
        expect_any_instance_of(VAProfile::ContactInformation::V1::Service).to receive(:get_person).twice
    ...
    context 'when there is cached data' do
      it 'returns the cached data', :aggregate_failures do
        ...
        expect_any_instance_of(VAProfile::ContactInformation::V1::Service).not_to receive(:get_person)
        expect(contact_info.response.person).to have_deep_attributes(person)
      end
    end
  end

...
RSpec.describe Dependents::Form686c674ConfirmationEmailJob, type: :job do
    ...
    context 'when an error occurs' do
      ...
      it 'logs the error and allows retry' do
        expect(Rails.logger).to receive(:warn).with(
          'Form686c674ConfirmationEmailJob failed, retrying send...',
  ...
  describe 'sidekiq_retries_exhausted' do
    it 'logs an error when retries are exhausted' do
      ...
      expect(Rails.logger).to receive(:error).with(
        'Form686c674ConfirmationEmailJob exhausted all retries',
        {

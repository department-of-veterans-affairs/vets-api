...
describe ClaimsApi::BD do
  ...
  let(:claim) do
    create(:auto_established_claim, evss_id: 600_400_688, id: '581128c6-ad08-4b1e-8b82-c3640e829fb3')
  end.freeze
  ...
  describe '#upload' do
    ...
    it 'uploads an attachment to BD for L023' do
      result = subject.send(:generate_upload_body, claim:, doc_type: 'L023', pdf_path:, original_filename: 'stuff.pdf')
  ...
  describe '#build_body' do
    it 'builds an L705 (5103) body correctly' do
      result = subject.send(:build_body, doc_type: 'L705', file_name: '5103.pdf', claim_id: claim.id)

      expected = { data: { systemName: 'VA.gov', docType: 'L705', claimId: '581128c6-ad08-4b1e-8b82-c3640e829fb3',
                           fileName: '5103.pdf', trackedItemIds: nil } }
    ...
    it 'builds an L122 (526) body correctly' do
      ...
      expected = { data: { systemName: 'VA.gov', docType: 'L122', claimId: '581128c6-ad08-4b1e-8b82-c3640e829fb3',
                           fileName: '21-526EZ.pdf', trackedItemIds: nil } }
      expect(result).to eq(expected)
    end
    ...
    it 'builds an L023 (corespondence) body correctly' do
      ...
      expected = { data: { systemName: 'VA.gov', docType: 'L023', claimId: '581128c6-ad08-4b1e-8b82-c3640e829fb3',
                           fileName: 'rx.pdf', trackedItemIds: nil } }
      expect(result).to eq(expected)
    end

    it 'handles empty params gracefully' do
      result = subject.send(:build_body, doc_type: 'L705', file_name: '5103.pdf', claim_id: claim.id,
                                         participant_id: nil, tracked_item_ids: nil, file_number: nil)
      expected = { data: { systemName: 'VA.gov', docType: 'L705', claimId: '581128c6-ad08-4b1e-8b82-c3640e829fb3',
                           fileName: '5103.pdf', trackedItemIds: nil } }
      expect(result).to eq(expected)
    end

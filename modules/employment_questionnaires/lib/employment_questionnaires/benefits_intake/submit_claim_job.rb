...
module EmploymentQuestionnaires
  module BenefitsIntake
    ...
    class SubmitClaimJob
      ...
      # Error if "Unable to find EmploymentQuestionnaires::SavedClaim"
      class EmploymentQuestionnairesBenefitIntakeError < StandardError; end

      # retry for  2d 1h 47m 12s
      # https://github.com/sidekiq/sidekiq/wiki/Error-Handling
      sidekiq_options retry: 16, queue: 'low'
      sidekiq_retries_exhausted do |msg|
        ia_monitor = EmploymentQuestionnaires::Monitor.new
        begin
          claim = EmploymentQuestionnaires::SavedClaim.find(msg['args'].first)
        rescue
          claim = nil
        end
      ...
      def perform(saved_claim_id, user_account_uuid = nil)
        return unless Flipper.enabled?(:employment_questionnaires_form_enabled)
      ...
      def init(saved_claim_id, user_account_uuid)
        ...
        @claim = EmploymentQuestionnaires::SavedClaim.find(saved_claim_id)
        unless @claim
          raise EmploymentQuestionnairesBenefitIntakeError,
                "Unable to find EmploymentQuestionnaires::SavedClaim #{saved_claim_id}"
        end
      ...
      def upload_document
        ...
        raise EmploymentQuestionnairesBenefitIntakeError, response.to_s unless response.success?
      end
      ...
      def send_submitted_email
        EmploymentQuestionnaires::NotificationEmail.new(@claim.id).deliver(:submitted)
      rescue => e

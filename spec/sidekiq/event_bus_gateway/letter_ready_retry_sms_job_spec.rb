...
RSpec.describe EventBusGateway::LetterReadyRetrySmsJob, type: :job do
  ...
  context 'when an error does not occur' do
    ...
    it 'sends an sms using VA Notify and updates the existing EventBusGatewayNotification' do
      expected_args = {
      ...
      expect(va_notify_service).to receive(:send_sms).with(expected_args)
  ...
  context 'when a VA Notify error occurs during sms sending' do
    before do
      allow(VaNotify::Service).to receive(:new).and_return(va_notify_service)
      allow(va_notify_service).to receive(:send_sms).and_raise(StandardError, 'VA Notify sms error')
      allow(Rails.logger).to receive(:error)
      allow(StatsD).to receive(:increment)
    end

    let(:error_message) { 'LetterReadyRetrySmsJob sms error' }
    let(:message_detail) { 'VA Notify sms error' }
    let(:tags) { EventBusGateway::Constants::DD_TAGS + ["function: #{error_message}"] }

    it 'does not send an sms successfully, logs the error, increments the statsd metric, and re-raises for retry' do
      expect(Rails.logger)
        .to receive(:error)
      ...
      # Notification should remain unchanged since sms send failed
      existing_notification.reload
  ...
  context 'when notification record is not found' do
    ...
    it 'raises EventBusGatewayNotificationNotFoundError, logs failure, and re-raises for retry' do
      ...
      expect(Rails.logger).to receive(:error)
        .with('LetterReadyRetrySmsJob sms error',
              { message: match(/EventBusGatewayNotificationNotFoundError/) })
      expect(StatsD).to receive(:increment)
        .with("#{described_class::STATSD_METRIC_PREFIX}.failure",
              tags: EventBusGateway::Constants::DD_TAGS + ['function: LetterReadyRetrySmsJob sms error'])

      expect do
        subject.new.perform(participant_id, template_id, personalisation, non_existent_id)
      end.to raise_error(EventBusGateway::LetterReadyRetrySmsJob::EventBusGatewayNotificationNotFoundError)
    end
  ...
  context 'when sidekiq retries are exhausted' do
    ...
    it 'logs the exhausted retries and increments the statsd metric' do
      ...
      expect(Rails.logger).to receive(:error)
        .with('LetterReadyRetrySmsJob retries exhausted', {
                job_id:,

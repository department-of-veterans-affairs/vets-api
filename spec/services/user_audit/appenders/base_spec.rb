...
RSpec.describe UserAudit::Appenders::Base do
  ...
  describe '#should_log?' do
    ...
    context 'when a required key is missing' do
      ...
      let(:expected_log_message) { 'Missing required log payload keys: user_verification' }
      ...
      it 'logs an error and returns false' do
        ...
        expect(Rails.logger).to have_received(:error).with(
          "#{appender.class.name} error: #{expected_log_message}",
  ...
  describe '#log' do
    ...
    context 'when append_log raises an error' do
      let(:error_message) { 'append failure' }
      let(:expected_log_message) { 'Error appending log' }
      let(:expected_log_payload) do
        {
          audit_log: { log_payload: payload, log_tags: named_tags },
          error_message:
      ...
      before do
        allow(appender).to receive(:append_log).and_raise(StandardError, error_message) # rubocop:disable RSpec/SubjectStub
      end

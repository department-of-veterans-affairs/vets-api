...
class Form526InProgressFormModifier
  ...
  def perform(ipf_id_array)
    ...
    raise ArgumentError, "All provided In-progress forms must be of type #{FORM_ID}" unless in_progress_forms.pluck(:form_id).uniq == [FORM_ID]
    ...
    in_progress_forms.each do |in_progress_form|
      id = in_progress_form.id
      form_parsed = JSON.parse(in_progress_form.form_data)
      did_4142_auth = form_parsed.dig("view:patient_acknowledgement", "view:acknowledgement")
    end

  ...
  describe 'get drive time' do
    it 'can be retrieved with an address' do
      VCR.use_cassette('bing/geocoding/vha_402',
        ...
        expect(json['data'][0]['attributes']['drivetime_band_min']).to eq(0)
        expect(json['data'][0]['attributes']['drivetime_band_max']).to eq(10)
        expect(json['data'][0]['id']).to eq('vha_402')
        expect(json['data'][0]['relationships']['va_facilities']['links']['related'])

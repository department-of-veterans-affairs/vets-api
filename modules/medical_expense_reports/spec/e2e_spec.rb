...
RSpec.describe 'Income and Assets End to End', skip: 'TODO after schema built', type: :request do
  let(:form) { build(:medical_expense_reports_claim) }
  ...
  it 'successfully completes the submission process' do
    # form submission
    expect(MedicalExpenseReports::SavedClaim).to receive(:new).with(form: form.form).and_call_original
    ...
    post '/medical_expense_reports/v0/claims', params: { param_name => { form: form.form } }
    ...
    medical_expense_reports_claim = MedicalExpenseReports::SavedClaim.find(saved_claim_id)
    expect(medical_expense_reports_claim).to be_present
    expect(medical_expense_reports_claim.confirmation_number).to eq data['attributes']['confirmation_number']
    ...
    email = MedicalExpenseReports::NotificationEmail.new(saved_claim_id)
    allow(MedicalExpenseReports::NotificationEmail).to receive(:new).and_return(email)
    ...
    lh_bi_uuid = MedicalExpenseReports::BenefitsIntake::SubmitClaimJob.new.perform(saved_claim_id)

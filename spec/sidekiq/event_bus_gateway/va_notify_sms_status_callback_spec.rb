...
describe EventBusGateway::VANotifySmsStatusCallback do
  describe '#call' do
    context 'notification callback' do
      ...
      context 'permanent-failure' do
        ...
        it 'logs error and increments StatsD' do
          ...
          expect(Rails.logger).to have_received(:error).with(
            'EventBusGateway::VANotifySmsStatusCallback',
      ...
      context 'temporary-failure' do
        ...
        context 'when event_bus_gateway_retry_sms is disabled' do
          before do
            allow(Flipper).to receive(:enabled?).with(:event_bus_gateway_retry_sms).and_return(false)
          end

          it 'does not attempt to send the sms again' do
            expect(EventBusGateway::LetterReadyRetrySmsJob).not_to receive(:perform_in)
            described_class.call(notification_record)
          end
        ...
        context 'when event_bus_gateway_retry_sms is enabled' do
          ...
          before do
            allow(Flipper).to receive(:enabled?).with(:event_bus_gateway_retry_sms).and_return(true)
            allow_any_instance_of(MPI::Service).to receive(:find_profile_by_identifier).and_return(mpi_profile_response)
            ebg_noti
          end

          it 'attempts to send the sms again using LetterReadyRetrySmsJob' do
            ...
            expect(EventBusGateway::LetterReadyRetrySmsJob).to receive(:perform_in).with(
              1.hour,
          ...
          it 'increments queued retry success metric' do
            allow(EventBusGateway::LetterReadyRetrySmsJob).to receive(:perform_in)
            allow(StatsD).to receive(:increment)
            described_class.call(notification_record)
            expect(StatsD).to have_received(:increment)
          ...
          context 'when max attempts exceeded' do
            before do
              ebg_noti.update!(attempts: EventBusGateway::Constants::MAX_SMS_ATTEMPTS + 1)
            end
            ...
            it 'does not queue retry job and logs exhausted retries' do
              ...
              expect(Rails.logger).to receive(:error).with(
                'EventBusGateway sms retries exhausted',
                { ebg_notification_id: ebg_noti.id,
                  max_attempts: EventBusGateway::Constants::MAX_SMS_ATTEMPTS }
              )
            ...
            it 'raises EventBusGatewayNotificationNotFoundError and increments failure metric' do
              expect(StatsD).to receive(:increment)
                .with("#{described_class::STATSD_METRIC_PREFIX}.queued_retry_failure",
                      tags: EventBusGateway::Constants::DD_TAGS + ['function: EventBusGateway::VANotifySmsStatusCallback::EventBusGatewayNotificationNotFoundError'])

              expect { described_class.call(notification_record) }
                .to raise_error(EventBusGateway::VANotifySmsStatusCallback::EventBusGatewayNotificationNotFoundError)
            end
          end
          ...
          context 'when MPI lookup fails' do
            ...
            it 'raises MPIError and increments failure metric' do
              expect(StatsD).to receive(:increment)
                .with("#{described_class::STATSD_METRIC_PREFIX}.queued_retry_failure",
                      tags: EventBusGateway::Constants::DD_TAGS + ['function: EventBusGateway::VANotifySmsStatusCallback::MPIError'])

              expect { described_class.call(notification_record) }
                .to raise_error(EventBusGateway::VANotifySmsStatusCallback::MPIError)
            end
          end
          ...
          context 'when first name is missing' do
            ...
            it 'raises MPINameError and increments failure metric' do
              expect(StatsD).to receive(:increment)
                .with("#{described_class::STATSD_METRIC_PREFIX}.queued_retry_failure",
                      tags: EventBusGateway::Constants::DD_TAGS + ['function: EventBusGateway::VANotifySmsStatusCallback::MPINameError'])

              expect { described_class.call(notification_record) }
                .to raise_error(EventBusGateway::VANotifySmsStatusCallback::MPINameError)
            end
          end
        end
      end

      ...
      it 'creates a new appointment' do
        VCR.use_cassette('vass/oauth_token_success') do
          VCR.use_cassette('vass/appointments/save_appointment_success') do
            ...
            expect(result['success']).to be true
            expect(result['data']['appointment_id']).to eq('appt-abc123')
          end
        end
      end
    end
      ...
      it 'retrieves a specific appointment' do
        VCR.use_cassette('vass/oauth_token_success') do
          VCR.use_cassette('vass/appointments/get_appointment_success') do
            ...
            expect(result['data']['agent_nickname']).to eq('Dr. Smith')
          end
        end
      end
    end
      ...
      it 'retrieves available agent skills' do
        VCR.use_cassette('vass/oauth_token_success') do
          VCR.use_cassette('vass/get_agent_skills_success') do
            ...
            expect(result['data']['agent_skills'].length).to eq(4)
            expect(result['data']['agent_skills'].first['skill_name']).to eq('Mental Health Counseling')
          end
        end
      end
    end

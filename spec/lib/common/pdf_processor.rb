...
RSpec.describe Common::PdfProcessor do
  ...
  describe '#process!' do
    context 'with a fresh file upload' do
      ...
      it 'accepts custom validator options' do
        processor = described_class.new(
          fresh_attachment,
          validator_options: { size_limit_in_bytes: 50.megabytes }
        )
        expect(processor).to be_a(described_class)
      end
    ...
    context 'with encrypted PDF and wrong password' do
      ...
      it 'raises ValidationError with password message' do
        expect { processor.process! }
          .to raise_error(Common::PdfProcessor::ValidationError) do |error|
    ...
    context 'cleanup of decrypted files' do
      ...
      it 'cleans up decrypted temp file even when validation fails' do
        ...
        expect { processor.process! }.to raise_error(Common::PdfProcessor::ValidationError)
    ...
    context 'testing conversion errors' do
      ...
      it 'raises ConversionError when conversion fails' do
        error = nil
        expect { processor.process! }
          .to raise_error(Common::PdfProcessor::ConversionError) { |e| error = e }
    ...
    context 'testing validation errors' do
      ...
      it 'raises ValidationError when PDF validation fails' do
        expect { processor.process! }
          .to raise_error(Common::PdfProcessor::ValidationError)

        begin
          processor.process!
        rescue Common::PdfProcessor::ValidationError => e
      ...
      it 'stores warnings on attachment if it responds to warnings' do
        attachment_with_warnings = PersistentAttachments::VAForm.new.tap do |att|
          att.form_id = '21-0779'
          att.file = File.open(pdf_path, 'rb')
        end

        processor = described_class.new(attachment_with_warnings)

        expect { processor.process! }.to raise_error(Common::PdfProcessor::ValidationError)
        expect(attachment_with_warnings.warnings).to include('File too large', 'Invalid format')
      end

      it 'does not raise error when attachment does not respond to warnings' do
        attachment_without_warnings = double(
          'Attachment',
          guid: 'test-guid',
          file: File.open(pdf_path, 'rb'),
          save: true
        )
        allow(attachment_without_warnings).to receive(:file=)
        allow(attachment_without_warnings).to receive(:respond_to?).with(:warnings).and_return(false)

        processor = described_class.new(attachment_without_warnings)

        expect { processor.process! }.to raise_error(Common::PdfProcessor::ConversionError)
      end
    end

    context 'with custom validator options' do
      let(:attachment) do
        PersistentAttachments::VAForm.new.tap do |att|
          att.form_id = '21-0779'
          att.file = File.open(pdf_path, 'rb')
      ...
      it 'merges custom options with defaults' do
        custom_options = { size_limit_in_bytes: 50.megabytes }
        processor = described_class.new(attachment, validator_options: custom_options)
        ...
        ).and_return(double(validate: double(valid_pdf?: true, errors: [])))

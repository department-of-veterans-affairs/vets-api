...
describe Mobile::V0::Adapters::Appeal, :aggregate_failures do
  ...
  context 'when issue descriptions are blank or nil' do
    ...
    it 'uses correct message format for higherLevelReview appeal type' do
    ...
    it 'uses correct message format for supplementalClaim appeal type' do
      # Create a supplementalClaim appeal by modifying the top-level type
      appeal_data = appeals.find { |a| a['id'] == '3294289' }.dup
      appeal_data['type'] = 'supplementalClaim'
      appeal_data['attributes']['issues'] = [
        { 'active' => true, 'date' => '2016-05-03',
          'description' => nil,
          'diagnosticCode' => '8100', 'lastAction' => 'remand' }
      ...
      expect(appeal.issues[0].description).to eq("We're unable to show this issue on your Supplemental Claim")
    end
  end
end

  ...
  module FesService
    class Base
      ...
      def validate(claim, claim_data)
        ...
        # The data is already prepared by the mapper, just wrap it
      ...
      def access_token
        @auth_token ||= token_service.get_token
      end

      def token_service
        auth_config = Settings.claims_api.fes.auth.ccg

        raise StandardError, 'FES auth configuration missing' if auth_config.blank?
        raise StandardError, 'FES RSA key must be provided' if auth_config.rsa_key.blank?

        Auth::ClientCredentials::Service.new(
          auth_config.aud_claim_url || "#{Settings.claims_api.fes.host}/oauth2/exemplar/system/v1/token",
          %w[documents.write documents.read],
          auth_config.client_id,
          Settings.claims_api.fes.host,
          auth_config.rsa_key,
          'claims_api:fes'
        )
      end

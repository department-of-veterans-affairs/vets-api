...
module Common
  module Exceptions
    ...
    # in one or more data sources (e.g., one backend returning errors while another succeeds)
    class UpstreamPartialFailure < ServiceError
      ...
      def build_default_detail
        if @failed_sources.any?
          "Partial data failure: some data could not be retrieved from #{@failed_sources.join(', ')}"
        else
          'Partial data failure: some data could not be retrieved from upstream sources'
        end
      end
    end
  end

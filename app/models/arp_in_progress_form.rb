...
class ArpInProgressForm < ApplicationRecord
  ...
  after_create ->(ipf) { StatsD.increment('arp_in_progress_form.create', tags: ["form_id:#{ipf.form_id}"]) }
  after_destroy ->(ipf) { StatsD.increment('arp_in_progress_form.destroy', tags: ["form_id:#{ipf.form_id}"]) }
  after_destroy lambda { |ipf|
                  StatsD.measure('arp_in_progress_form.lifespan', Time.current - ipf.created_at,
                                 tags: ["form_id:#{ipf.form_id}"])
                }
  after_save :log_hca_email_diff

  def self.form_for_user(form_id, user)
    user_uuid_form = ArpInProgressForm.find_by(form_id:, user_uuid: user.uuid)
    user_account_form = ArpInProgressForm.find_by(form_id:, user_account: user.user_account) if user.user_account
    user_uuid_form || user_account_form
  end

  def self.for_user(user)
    user_uuid_forms = ArpInProgressForm.where(user_uuid: user.uuid)
    if user.user_account
      user_uuid_forms.or(ArpInProgressForm.where(user_account: user.user_account))
    else
      user_uuid_forms
    end
  end
  ...
  def expires_after
    @expires_after ||=
      if Flipper.enabled?(:arp_in_progress_form_custom_expiration)

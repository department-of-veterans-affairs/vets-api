...
RSpec.describe EmploymentQuestionairres::BenefitsIntake::SubmissionHandler do
  ...
  let(:notification) { double(EmploymentQuestionairres::NotificationEmail) }
  ...
  before do
    ...
    allow(EmploymentQuestionairres::NotificationEmail).to receive(:new).with(claim.id).and_return notification
  end

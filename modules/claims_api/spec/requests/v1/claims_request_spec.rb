  ...
  it 'lists all Claims', run_at: 'Tue, 12 Dec 2017 03:09:06 GMT' do
    with_okta_user(scopes) do |auth_header|
      ...
      VCR.use_cassette('evss/claims/claims') do
        get '/services/claims/v0/claims', nil, request_headers.merge(auth_header)
  ...
  context 'for a single claim' do
    it 'shows a single Claim', run_at: 'Wed, 13 Dec 2017 03:28:23 GMT' do
      with_okta_user(scopes) do |auth_header|
        VCR.use_cassette('evss/claims/claim') do
          get '/services/claims/v0/claims/600118851', nil, request_headers.merge(auth_header)
    ...
    it 'shows a single Claim through auto established claims', run_at: 'Wed, 13 Dec 2017 03:28:23 GMT' do
      with_okta_user(scopes) do |auth_header|
        ...
        VCR.use_cassette('evss/claims/claim') do
          get '/services/claims/v0/claims/d5536c5c-0465-4038-a368-1a9d9daf65c9', nil, request_headers.merge(auth_header)
          expect(response).to match_response_schema('claims_api/claim')
        end
      end
    end
    ...
    it 'should user the poa verifier when the header is present' do
      with_okta_user(scopes) do |auth_header|
        VCR.use_cassette('evss/claims/claim') do
          ...
          get '/services/claims/v0/claims/d5536c5c-0465-4038-a368-1a9d9daf65c9', nil, headers
    ...
    VALID_HEADERS.each_key do |header|
      context "without #{header}" do
        it 'returns a bad request response' do
          with_okta_user(scopes) do |auth_header|
            VCR.use_cassette('evss/claims/claims') do
              get '/services/claims/v0/claims', nil, VALID_HEADERS.merge(auth_header).except(header)

  ...
  describe 'getting data' do
    context 'with a successful response' do
      ...
      it 'has the proper transaction date' do
        expect(response.items.first.transaction_date).to be_a(Date).and eq(Date.parse('2016-05-18'))
      end
      ...
      context 'with a location' do
        ...
        it 'has the right segment identifier' do
          expect(location.segment_identifier).to be_a(String).and eq('1')
        end

        it 'has the right country' do
          expect(location.country).to be_a(String).and eq('AE')
        end

        it 'has the right ISO Alpha3 country' do
          expect(location.iso_alpha3_country).to be_a(String).and eq('ARE')
        end

        it 'has the right begin date' do
          expect(location.begin_date).to be_a(Date).and eq(Date.parse('2008-09-08'))
        end

        it 'has the right end date' do
          expect(location.end_date).to be_a(Date).and eq(Date.parse('2009-04-18'))
        end

        it 'has the right termination reason code' do
          expect(location.termination_reason_code).to be_a(String).and eq('C')
        end

        it 'has the right transaction date' do
          expect(location.transaction_date).to be_a(Date).and eq(Date.parse('2016-05-14'))
        end
      end
    end
  end

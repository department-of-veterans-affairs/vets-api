...
RSpec.describe CovidResearch::Volunteer::GenisisService do
  ...
  let(:intake_form_data_v2) { read_fixture('valid-intake-submission-v2.json') }
  ...
  describe 'intake V2' do
    let(:subject) { described_class.new(intake_form_data_v2, serializer) }

    describe 'prep' do
      it 'serializes the data to build the genISIS payload' do
        expect(serializer).to receive(:serialize).with(JSON.parse(intake_form_data_v2))

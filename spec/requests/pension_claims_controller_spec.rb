...
RSpec.describe 'Pension Claim Integration', type: %i[request serializer] do
  ...
  describe 'POST create' do
    ...
    #     expect(Rails.logger).to receive(:info).with(/^Begin ClaimGUID=\S+ Form=21P-527EZ-ARE UserID=/)
    ...
    context 'with valid params' do
      ...
      it 'logs the successful submission' do
        expect(Rails.logger).to receive(:info).with(/^Begin ClaimGUID=\S+ Form=21P-527EZ-ARE UserID=/)
        expect(Rails.logger).to receive(:info).with(/Submitted job ClaimID=\S+ Form=21P-527EZ-ARE UserID=/)
        subject
      end
    end
  end

...
RSpec.describe KmsKeyRotation::BatchInitiatorJob, type: :job do
  ...
  before do
    ...
    KmsKeyRotation::BatchInitiatorJob.send(:remove_const, :CHUNK_SIZE)
    KmsKeyRotation::BatchInitiatorJob.const_set(:CHUNK_SIZE, 5)
  ...
  describe '#perform' do
    ...
    it 'batches jobs for records needing rotation' do
      ...
      expect(KmsKeyRotation::RotateKeysJob.jobs.size).to eq(8)
    end
    ...
    it 'creates RotateKeysJob jobs with batched_gids as args' do
      ...
      expect(rotate_jobs.size).to eq(8)

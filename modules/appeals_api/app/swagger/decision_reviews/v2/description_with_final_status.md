...
| uploaded  | Indicates document submission has been successfully uploaded (PUT) to the API server and is waiting to be sent to VA's mail handling system.<br /><br /> Submission has not yet been validated.<br> Any errors with the evidence submission, such as having an unreadable PDF, may cause an Error status. |

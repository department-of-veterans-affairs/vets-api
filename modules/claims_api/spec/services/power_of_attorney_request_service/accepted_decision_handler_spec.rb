...
describe ClaimsApi::PowerOfAttorneyRequestService::AcceptedDecisionHandler do
  ...
  describe '#gather_veteran_data' do
    let(:metadata) do
      { 'veteran' => { 'vnp_phone_id' => '106175', 'vnp_email_id' => '148885', 'vnp_mail_id' => '148886' } }
    end
    let(:claimant) { {} }
    let(:expected_response) do
      { 'name' => 'Ralph Lee', 'ssn' => '796378782', 'file_number' => '796378782', 'date_of_birth' => '1948-10-30' }
    end

    it 'compiles the veteran data object as expected' do
      subject.instance_variable_set(:@veteran, veteran)

      res = subject.send(:gather_veteran_data)

      expect(res).to eq(expected_response)
    end
  end

  describe '#gather_claimant_data' do
    let(:claimant) do
      OpenStruct.new(
        icn: '1013093331V548481',
        first_name: 'Wally',
    ...
    let(:expected_response) do
      { 'name' => 'Wally Morell', 'ssn' => '796378782', 'file_number' => '796378782', 'date_of_birth' => '1948-10-30' }
    end

    it 'compiles the veteran data object as expected' do
      subject.instance_variable_set(:@claimant, claimant)

      res = subject.send(:gather_claimant_data)

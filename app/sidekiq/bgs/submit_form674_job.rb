...
module BGS
  class SubmitForm674Job < Job
    ...
    def self.send_backup_submission(encrypted_user_struct_hash, vet_info, saved_claim_id, user_uuid)
      ...
      Rails.logger.warn('BGS::SubmitForm674Job backup submission receieved error!',
                        { error: e.message, nested_error: e.cause&.message })

      form = InProgressForm.where(form_id: FORM_ID, user_uuid:).first_or_initialize
      form.user_account = (generate_user_struct(encrypted_user_struct_hash, vet_info))
      claim = SavedClaim::DependencyClaim.find(saved_claim_id)
      form.update(form_data: claim.open_struct_form)
    end

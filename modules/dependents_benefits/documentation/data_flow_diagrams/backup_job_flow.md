...
graph TD
    ...
    MarkProcessing --> SendNotification[Send Submitted Notification<br/>Claim accepted by Lighthouse]
    SendNotification --> Cleanup[Cleanup Temp Files<br/>Delete form_path + attachment_paths PDFs]
    ...
    class Start trigger
    class LoadClaim,CreateRecords,GetUUID,PrepareSubmission,CollectClaims,ProcessPDFs,SetMainForm,CollectAttachments,GenerateMetadata,SendNotification,SendFailure,LogFailure,Complete,Cleanup,RetryJob,Exhausted process
...
- **Notifications**:
  - **Success**: Submitted notification (claim accepted by Lighthouse, awaiting VBMS processing)

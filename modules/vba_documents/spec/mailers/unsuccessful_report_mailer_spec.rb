...
RSpec.describe VBADocuments::UnsuccessfulReportMailer, type: [:mailer] do
  ...
  describe '#build' do
    module MailerHelper
      ...
      def self.setup(current_env_url, totals, error_upload, uploaded_upload)
        ...
        VBADocuments::UnsuccessfulReportMailer.build(totals,
                                                     [error_upload],
                                                     [uploaded_upload], 7.days.ago,
                                                     Time.zone.now).deliver_now
      end
    end

    MailerHelper.env_hash.each_pair do |k, v|
      it "sends the email for #{k}" do
        email = MailerHelper.setup(v, totals, error_upload, uploaded_upload)

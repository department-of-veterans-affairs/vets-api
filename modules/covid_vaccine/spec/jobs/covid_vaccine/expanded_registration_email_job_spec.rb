  ...
  describe '#perform' do
    it 'logs message to sentry and returns if no submission exists' do
      with_settings(Settings.sentry, dsn: 'T') do
        ...
        subject.perform('non-existent-submission-id', email, date)
      end
    end
    ...
    context 'when an email confirmation ID exists' do
      ...
      it 'avoid sending an email if an email confirmation id is already present' do
        expect(VaNotify::Service).not_to receive(:new)
        subject.perform(registration_submission.id, email, date)
      end
    end
    ...
    context 'with an error response from VANotify' do
      it 'raises an exception' do
        ...
        expect { subject.perform(registration_submission.id, email, date) }.to raise_error(StandardError)
      end

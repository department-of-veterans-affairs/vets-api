...
RSpec.describe AccreditedRepresentativePortal::SubmitBenefitsIntakeClaimJob, :uploader_helpers do
  ...
  describe '#perform' do
    ...
    before do
      ...
      allow(job).to receive_messages(init: nil, process_record: pdf_path, create_form_submission_attempt: nil,
                                     send_confirmation_email: nil, cleanup_file_paths: nil)
    end
    ...
    context 'when email sending fails' do
      ...
      it 'logs the error but does not reraise' do
        ...
        expect(monitor)
          ...
          .with(claim, intake_service, user_account_uuid, 'confirmation', email_error)

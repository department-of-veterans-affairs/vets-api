...
RSpec.describe EmploymentQuestionairres::SavedClaim do
  subject { described_class.new }

  let(:instance) { build(:employment_questionairres_claim) }
  ...
  it 'inherits init callsbacks from saved_claim' do
    expect(subject.form_id).to eq(EmploymentQuestionairres::FORM_ID)
    expect(subject.guid).not_to be_nil
    expect(subject.type).to eq(EmploymentQuestionairres::SavedClaim.to_s)
  end
  ...
  describe '#send_email' do
    it 'calls EmploymentQuestionairres::NotificationEmail with the claim id and delivers the email' do
      claim = build(:employment_questionairres_claim)
      email_type = :error
      klass = EmploymentQuestionairres::NotificationEmail

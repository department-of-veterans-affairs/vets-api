...
RSpec.describe EVSS::DisabilityCompensationForm::SubmitUploads, type: :job do
  ...
  before do
    ...
    allow(Flipper).to receive(:enabled?).with(:disability_compensation_use_api_provider_for_submit_veteran_upload)
                                        .and_return(false)
    allow(Flipper).to receive(:enabled?).with('disability_compensation_use_api_provider_for_submit_veteran_upload',
                                              instance_of(User)).and_return(false)
    allow(Flipper).to receive(:enabled?).with(:form526_send_document_upload_failure_notification).and_return(false)
        ...
        context 'when the form526_send_document_upload_failure_notification Flipper is enabled' do
          before do
            allow(Flipper).to receive(:enabled?)
              .with(:form526_send_document_upload_failure_notification).and_return(true)
          end
  ...
  describe 'When an ApiProvider is used for uploads' do
    ...
    context 'when file_data exists' do
      ...
      context 'when the disability_compensation_upload_veteran_evidence_to_lighthouse flipper is enabled' do
        ...
        before do
          allow(Flipper).to receive(:enabled?).with('disability_compensation_upload_veteran_evidence_to_lighthouse',
  ...
  context 'catastrophic failure state' do
    ...
    describe 'when an error occurs during exhaustion handling and FailureEmail fails to enqueue' do
      ...
      before do
        allow(Flipper).to receive(:enabled?).with(:form526_send_document_upload_failure_notification).and_return(true)

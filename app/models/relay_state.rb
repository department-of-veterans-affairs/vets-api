...
class RelayState
  ...
  validates :relay_enum, allow_blank: true, inclusion: { in: RELAY_KEYS, message: '[%{value}] is not a valid relay enum' }
  validates :url, allow_blank: true, inclusion: { in: ALL_RELAY_URLS, message: '[%{value}] is not a valid relay url' }
  ...
  def initialize(relay_enum: nil, url: nil)
    ...
    log_message_to_sentry('Invalid SAML RelayState!', :error, url_whitelist: ALL_RELAY_URLS, enum_whitelist: RELAY_KEYS) unless valid?
  end

  def login_url
    return default_login_url if @relay_enum.blank? && @url.blank?
    get_url(:relays, LOGIN_URLS)
  end

  def logout_url
    return default_logout_url if @relay_enum.blank? && @url.blank?
    get_url(:logout_relays, LOGOUT_URLS)
  end

  private

  def get_url(type = :relays, valid_urls)

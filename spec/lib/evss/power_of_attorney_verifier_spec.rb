...
describe EVSS::PowerOfAttorneyVerifier do
  ...
  it 'should not raise an exception if poa matches' do
    FactoryBot.create(:representative, poa: 'A1Q', first_name: identity.first_name, last_name: identity.last_name)
    expect do
  ...
  it 'should raise an exception if poa does not matches' do
    FactoryBot.create(:representative, poa: 'B1Q', first_name: identity.first_name, last_name: identity.last_name)
    expect do
      EVSS::PowerOfAttorneyVerifier.new(user).verify(identity)

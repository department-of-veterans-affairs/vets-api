...
RSpec.describe SavedClaim::EducationBenefits::VA10297 do
  let(:instance) { build(:va10297_full_form) }
  ...
  describe '#after_submit' do
    ...
    describe 'confirmation email for 10297' do
      it 'is skipped when feature flag is turned off' do
        Flipper.disable(:form10297_confirmation_email)
        allow(VANotify::EmailJob).to receive(:perform_async)

        subject = create(:va10297_full_form)
        subject.after_submit(user)

        expect(VANotify::EmailJob).not_to have_received(:perform_async)
        Flipper.enable(:form10297_confirmation_email)
      end
      ...
      it 'sends v2 when they have not applied for VA education benefits previously' do
        ...
        subject = create(:va10297_no_education_benefits)
        confirmation_number = subject.education_benefits_claim.confirmation_number

        subject.after_submit(user)
        expect(VANotify::EmailJob).to have_received(:perform_async).with(
          'test@test.com',
          'form10297_extra_action_confirmation_email_template_id',
          {
            'first_name' => 'TEST',
            'date_submitted' => Time.zone.today.strftime('%B %d, %Y'),
            'confirmation_number' => confirmation_number,

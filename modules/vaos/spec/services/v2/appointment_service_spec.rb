...
describe VAOS::V2::AppointmentsService do
  ...
  shared_context 'appointments service flipper setup' do |parallel_api_calls_enabled|
    before do
      ...
      allow(Flipper).to receive(:enabled?).with(:appointments_parallel_api_calls,
                                                user).and_return(parallel_api_calls_enabled)
    end
  end
  ...
  [false, true].each do |parallel_api_calls_enabled|
    context "with appointments_parallel_api_calls #{parallel_api_calls_enabled ? 'enabled' : 'disabled'}" do
      include_context 'appointments service flipper setup', parallel_api_calls_enabled
        ...
        context 'using VAOS' do
          ...
          context 'when the patientIcn is missing' do
            it 'raises a backend exception' do
              VCR.use_cassette('vaos/v2/appointments/post_appointments_400',
                               match_requests_on: %i[method path query]) do
          ...
          context 'when the upstream server returns a 500' do
            it 'raises a backend exception' do
              VCR.use_cassette('vaos/v2/appointments/post_appointments_500',
                               match_requests_on: %i[method path query]) do
                expect { subject.post_appointment(community_cares_request_body) }.to raise_error(
                  Common::Exceptions::BackendServiceException
                )
        ...
        context 'using VAOS' do
          ...
          context 'when requesting a list of appointments given a date range' do
            it 'returns a 200 status with list of appointments' do
              VCR.use_cassette('vaos/v2/appointments/get_appointments_200_with_facilities_200',
                               match_requests_on: %i[method path query], allow_playback_repeats: true,
                               tag: :force_utf8) do
            ...
            it 'returns with list of appointments and appends local start time' do
              allow_any_instance_of(VAOS::V2::MobileFacilityService)
                .to receive(:get_facility!).and_return(mock_facility2)
              VCR.use_cassette('vaos/v2/appointments/get_appointments_200_with_facilities_200',
                               match_requests_on: %i[method path query], allow_playback_repeats: true,
                               tag: :force_utf8) do
                response = subject.get_appointments(start_date2, end_date2)
          ...
          context 'when partial success is returned and failures are returned with ICNs' do
            ...
            it 'returns partial error message' do
              VCR.use_cassette('vaos/v2/appointments/get_appointments_200_with_facilities_200_and_log_data',
                ...
                expect(partial_error_message[:request][:body])
                  .to eq('We’re working to fix this problem. To reschedule ' \
                         'a request that’s not in this list, contact the ' \
                         'VA facility where it was requested.')
                expect(partial_error_message[:booked][:title])
                  .to eq('We can’t show some of your appointments right now.')
                expect(partial_error_message[:booked][:body])
                  .to eq('We’re working to fix this problem. To manage an ' \
          ...
          context 'when requesting a list of appointments given a date range and single status' do
            it 'returns a 200 status with list of appointments' do
              VCR.use_cassette('vaos/v2/appointments/get_appointments_single_status_200',
                               allow_playback_repeats: true, match_requests_on: %i[method path query],
                               tag: :force_utf8) do
          ...
          context 'includes travel claims' do
            ...
            context 'with parallel execution feature toggle' do
              it 'successfully processes appointments when parallel toggle is enabled' do
              ...
              it 'successfully processes appointments when parallel toggle is disabled' do
                ...
                VCR.use_cassette('travel_pay/200_search_claims_by_appt_date_range',
                  ...
                  VCR.use_cassette('vaos/v2/appointments/get_appointments_200_with_facilities_200',
                                   allow_playback_repeats: true, match_requests_on: %i[method path],
                                   tag: :force_utf8) do
                    response = subject.get_appointments(start_date2, end_date2, nil, {}, { travel_pay_claims: true })
        ...
        context 'when a MAP token error occurs' do
          ...
          it 'logs missing ICN error' do
            ...
            expect(Rails.logger).to have_received(:warn).with(
              'VAOS::V2::AppointmentService#get_appointments missing ICN'
            )
      ...
      describe '#get_appointment' do
          ...
          context 'with an appointment' do
            context 'with Jacqueline Morgan' do
              it 'returns a proposed appointment' do
                allow_any_instance_of(VAOS::V2::MobileFacilityService)
                  .to receive(:get_facility!).and_return(mock_facility)
        ...
        context 'using VPG' do
          ...
          context 'when the upstream server returns a 500' do
            it 'raises a backend exception' do
              VCR.use_cassette('vaos/v2/appointments/get_appointment_500_vpg',
                               match_requests_on: %i[method path query]) do
                expect { subject.get_appointment('00000') }.to raise_error(
                  Common::Exceptions::BackendServiceException
                )
        ...
        context 'when the upstream server attempts to cancel an appointment' do
          context 'with Jaqueline Morgan' do
            ...
            context 'using vaos-service' do
              ...
              it 'returns a 400 when the appointment is not cancellable' do
                VCR.use_cassette('vaos/v2/appointments/cancel_appointment_400',
                                 match_requests_on: %i[method path query]) do
                  expect { subject.update_appointment('42081', 'cancelled') }
                    .to raise_error do |error|
                    expect(error).to be_a(Common::Exceptions::BackendServiceException)
      ...
      describe '#get_appointments merge' do
        ...
        context 'when include eps is true' do
          ...
          it 'merges eps appointments with vaos appointments and removes eps appointment with duplicate ' \
             'referralNumbers but not the vaos appointment' do
            VCR.use_cassette('vaos/eps/get_vaos_appointments_200_with_merge',
                             match_requests_on: %i[method path query], allow_playback_repeats: true,
                             tag: :force_utf8) do
              allow_any_instance_of(Eps::AppointmentService).to receive(:get_appointments).and_return(eps_appointments)
            ...
            context 'when the call for vaos appointments returns a partial failure' do
              it 'logs the failures, anonymizes the ICNs sent to the log, and returns the failure messages' do
                VCR.use_cassette('vaos/v2/appointments/get_appointments_200_with_partial_errors_v2',
                  ...
                  expect(check[:failures][0][:message]).to eq(
                    'Could not fetch appointments from Vista Scheduling Provider'
                  )
                end
              end
            end
      ...
      describe '#set_modality' do
        ...
        it 'is vaInPerson for nil vvsKind and false vvsVistaVideoAppt' do
          appt = build(:appointment_form_v2, :va_proposed_valid_reason_code_text, :telehealth,
                       :vistaVideoFalse).attributes
      ...
      describe '#future' do
        ...
        it 'sets future to true if the appointment is not a request and occurs after the ' \
           'beginning of the current day' do
          appt = build(:appointment_form_v2, :va_proposed_valid_reason_code_text).attributes
          appt[:type] = 'VA'
        ...
        it 'sets future to false if the appointment is not a request and occurs before the ' \
           'beginning of the current day' do
          appt = build(:appointment_form_v2, :va_proposed_valid_reason_code_text).attributes
          appt[:type] = 'VA'
          appt[:start] = Time.now.utc - 1.day

...
describe AppealsApi::EvidenceSubmission::FileValidator do
  describe '#validate' do
    ...
    context 'when supported file uploaded' do
      it { expect(described_class.new(valid_pdf).call).to be true }
    end
    ...
    context 'when file size is too large' do
      ...
      let(:big_big_file) { instance_double('File', to_path: path, path: path, size: 200.megabytes) }

      it 'returns error data' do
        expect(described_class.new(big_big_file).call).to match(
          {

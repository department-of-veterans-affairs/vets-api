...
RSpec.describe EducationForm::CreateDailySpoolFiles, type: :model, form: :education_benefits do
  ...
  context '#format_application' do
    ...
    context 'with a 1995-STEM form' do
      let(:application_1606) { create(:va1995-STEM_full_form).education_benefits_claim }

      it 'tracks the 1995-STEM form' do
        expect(subject).to receive(:track_form_type).with('22-1995-STEM', 999)
        result = subject.format_application(application_1606, rpo: 999)
        expect(result).to be_a(EducationForm::Forms::VA1995-STEM)
      end
    end
  ...
  context '#perform' do
    context 'with a mix of valid and invalid record', run_at: '2016-09-16 03:00:00 EDT' do
      ...
      before do
        ...
        FactoryBot.create(:va1995-STEM_full_form)

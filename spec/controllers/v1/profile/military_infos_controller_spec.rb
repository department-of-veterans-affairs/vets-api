  ...
  describe '#show' do
    ...
    context 'when user does not have edipi' do
      ...
      it 'returns a status of 403' do
        VCR.use_cassette('va_profile/profile/v3/military_info_403') do
          get(:show)
        end
    ...
    context 'when user have a bad edipi' do
      let(:user) { create(:user, :loa3, edipi: '12345') }

      it 'returns a status of 403' do
        VCR.use_cassette('va_profile/profile/v3/military_info_404') do
          get(:show)
        end

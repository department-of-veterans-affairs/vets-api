  ...
  describe "when initialized" do
    context 'with a hash' do
      ...
      it 'should have a session object' do
        session_attributes = VaHealthcareMessaging::ClientSession.new(session_options).attributes
        expect(subject.session.attributes).to eq(session_attributes)
      end
    end

    context 'when initialized with objects' do
      let(:session) { VaHealthcareMessaging::ClientSession.new(session_options) }
      let(:config) { VaHealthcareMessaging::Configuration.new(config_options) }

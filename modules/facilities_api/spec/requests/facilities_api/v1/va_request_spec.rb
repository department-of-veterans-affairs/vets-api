...
RSpec.shared_examples 'paginated request from params with expected IDs' do |request_params, ids, mobile = nil|
  ...
  context request_params do
    ...
    it 'is expected to include pagination links' do
      ...
      prev_link = prev_page ? "http://localhost/facilities_api/v1/va?#{prev_params}" : nil
      next_link = next_page ? "http://localhost/facilities_api/v1/va?#{next_params}" : nil

      expect(parsed_body[:links]).to match(
        self: "http://localhost/facilities_api/v1/va?#{params.merge({ page: current_page, per_page: 10 }).to_query}",
        first: "http://localhost/facilities_api/v1/va?#{params.merge({ per_page: 10 }).to_query}",
        prev: prev_link,
        next: next_link,
        last: "http://localhost/facilities_api/v1/va?#{params.merge({ page: last_page, per_page: 10 }).to_query}"
      )
    end
  end
end

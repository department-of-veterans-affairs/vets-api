...
RSpec.describe AccreditedRepresentativePortal::V0::RepresentativeFormUploadController, type: :request do
  ...
  describe '#submit' do
    ...
    let(:veteran_params) { JSON.parse(representative_fixture_path.read).merge('confirmationCode' => attachment_guid) }
    ...
    let(:claimant_params) { JSON.parse(claimant_fixture_path.read).merge('confirmationCode' => attachment_guid) }

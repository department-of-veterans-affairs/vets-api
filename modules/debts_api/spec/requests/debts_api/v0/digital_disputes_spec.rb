  ...
  describe '#create' do
    context 'when digital_dmc_dispute_service flipper disabled' do
      ...
      before do
        ...
        allow(Flipper).to receive(:enabled?).with(:digital_dmc_dispute_service, user).and_return(false)
      end
    ...
    context 'when digital_dmc_dispute_service flipper enabled' do
      ...
      before do
        sign_in_as(user)
        allow(StatsD).to receive(:increment)
        allow(Flipper).to receive(:enabled?).with(:digital_dmc_dispute_service, user).and_return(true)

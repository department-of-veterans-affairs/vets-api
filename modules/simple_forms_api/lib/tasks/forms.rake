...
namespace :simple_forms_api do
  task :generate, [:form_path] => :environment do |_, args|
    ...
    new_model_file = Rails.root.join(SIMPLE_FORMS_API_MODELS_PATH, "#{form_name}.rb")
    ...
    mapping_file = Rails.root.join(SIMPLE_FORMS_API_MAPPINGS_PATH, "#{form_name}.json.erb")

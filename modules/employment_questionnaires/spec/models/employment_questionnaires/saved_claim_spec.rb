...
RSpec.describe EmploymentQuestionnaires::SavedClaim do
  subject { described_class.new }

  let(:instance) { build(:employment_questionnaires_claim) }
  ...
  it 'inherits init callsbacks from saved_claim' do
    expect(subject.form_id).to eq(EmploymentQuestionnaires::FORM_ID)
    expect(subject.guid).not_to be_nil
    expect(subject.type).to eq(EmploymentQuestionnaires::SavedClaim.to_s)
  end
  ...
  describe '#send_email' do
    it 'calls EmploymentQuestionnaires::NotificationEmail with the claim id and delivers the email' do
      claim = build(:employment_questionnaires_claim)
      email_type = :error
      klass = EmploymentQuestionnaires::NotificationEmail
      notification_double = instance_double(klass)

      expect(klass).to receive(:new).with(claim.id).and_return(notification_double)

...
RSpec.describe BEP::UploadedDocumentService do
  let(:user) { create(:evss_user, :loa3) }

  describe '#get_documents' do
    context 'with a valid participant id' do
      it 'returns a list of uploaded documents' do
        VCR.use_cassette('bep/uploaded_document_service/uploaded_document_data') do
          service = BEP::UploadedDocumentService.new(user)
    ...
    context 'with an invalid participant id' do
      it 'returns an error' do
        VCR.use_cassette('bep/uploaded_document_service/bad_participant_id') do

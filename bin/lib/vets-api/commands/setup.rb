  ...
  module Commands
    class Setup
      class << self
        ...
        def setup_native
          if RUBY_DESCRIPTION.include?('ruby 3.2.4')
            VetsApi::Setups::Native.new.run
          else
            puts "\nBefore continuing Ruby v3.2.4 must be installed"
        ...
        def setup_hybrid
          if RUBY_DESCRIPTION.include?('ruby 3.2.4') && docker_running?
            VetsApi::Setups::Hybrid.new.run
          else
            puts "\nBefore continuing Ruby v3.2.4 AND Docker Desktop (Engine + Compose) must be installed"

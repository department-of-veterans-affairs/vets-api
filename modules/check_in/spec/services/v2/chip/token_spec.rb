...
describe V2::Chip::Token do
  ...
  describe '.build' do
    it 'returns an instance of Token' do
      expect(subject.build).to be_an_instance_of(V2::Chip::Token)
    end
  end
  ...
  describe '#fetch' do
    ...
    before do
      allow_any_instance_of(V2::Chip::Request).to receive(:post).with(anything).and_return(faraday_response)
    end
  ...
  describe '#base_path' do
    it 'returns base_path' do
      expect(subject.build.base_path).to eq('dev')
    end
  end
end

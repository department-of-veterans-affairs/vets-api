...
require_relative '../../../../../lib/decision_review/higher_level_review/get_contestable_issues/response.rb'
require_relative '../../../../../lib/decision_review/response_schema_error.rb'
require_relative '../../../../../lib/decision_review/schema_error.rb'
require 'rails_helper'

describe DecisionReview::HigherLevelReview::GetContestableIssues::Response do
  let(:response) { Struct.new(:status, :body).new status, body }
  let(:body) { VetsJsonSchema::EXAMPLES["HLR-GET-CONTESTABLE-ISSUES-RESPONSE-#{status}"] }
  let(:status) { 200 }

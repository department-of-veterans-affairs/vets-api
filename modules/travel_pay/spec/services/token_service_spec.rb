...
describe TravelPay::TokenService do
  ...
  before do
    ...
    allow_any_instance_of(TravelPay::TokenService).to receive(:connection).and_return(conn)
  end
  ...
  context 'request_veis_token' do
    it 'returns veis token from proper endpoint' do
      ...
      token = token_service.request_veis_token
  ...
  context 'request_btsss_token' do
    ...
    it 'returns btsss token from proper endpoint' do
      ...
      token_service = TravelPay::TokenService.new
      token = token_service.request_btsss_token(user)
  ...
  context 'request_sts_token' do
    ...
    it 'builds sts assertion and requests sts token' do
      ...
      token_service = TravelPay::TokenService.new
      sts_token = token_service.request_sts_token(user)

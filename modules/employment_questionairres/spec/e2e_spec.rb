...
require 'employment_questionairres/benefits_intake/submit_claim_job'
...
RSpec.describe 'Income and Assets End to End', skip: 'TODO after schema built', type: :request do
  let(:form) { build(:employment_questionairres_claim) }
  let(:param_name) { :employment_questionairres_claim }
  let(:pdf_path) { 'random/path/to/pdf' }
  let(:monitor) { EmploymentQuestionairres::Monitor.new }
  let(:service) { BenefitsIntake::Service.new }

  let(:stats_key) { BenefitsIntake::SubmissionStatusJob::STATS_KEY }

  before do
    allow(EmploymentQuestionairres::Monitor).to receive(:new).and_return(monitor)
    allow(BenefitsIntake::Service).to receive(:new).and_return(service)

    allow(Flipper).to receive(:enabled?).with(anything).and_call_original
    allow(Flipper).to receive(:enabled?).with(:employment_questionairres_submitted_email_notification).and_return true
  ...
  it 'successfully completes the submission process' do
    # form submission
    expect(EmploymentQuestionairres::SavedClaim).to receive(:new).with(form: form.form).and_call_original
    ...
    post '/employment_questionairres/v0/claims', params: { param_name => { form: form.form } }
    ...
    employment_questionairres_claim = EmploymentQuestionairres::SavedClaim.find(saved_claim_id)
    expect(employment_questionairres_claim).to be_present
    expect(employment_questionairres_claim.confirmation_number).to eq data['attributes']['confirmation_number']
    ...
    email = EmploymentQuestionairres::NotificationEmail.new(saved_claim_id)
    allow(EmploymentQuestionairres::NotificationEmail).to receive(:new).and_return(email)
    ...
    lh_bi_uuid = EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob.new.perform(saved_claim_id)
    ...
    BenefitsIntake::SubmissionStatusJob.new.perform(EmploymentQuestionairres::FORM_ID)

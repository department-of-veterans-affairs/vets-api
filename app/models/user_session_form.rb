...
class UserSessionForm
  ...
  def user_attribute_mismatch_checks(saml_attributes)
    return unless saml_attributes.dig(:loa, :current) == LOA::THREE

    identifier_type = saml_attributes.dig(:sign_in, :service_name)
    saml_ssn = saml_attributes[:ssn]

    return if identifier_type == SAML::User::MHV_ORIGINAL_CSID
    return if saml_ssn.blank?

...
class MHVLoggingService
  def self.login(current_user)
    ...
    return false if current_user.mhv_correlation_id || current_user.mhv_last_signed_in
  ...
  def self.logout(current_user)
    ...
    return false unless current_user.mhv_correlation_id || current_user.mhv_last_signed_in

  ...
  describe '#index' do
    context 'successful response from API' do
      ...
      it 'responds with 200' do
        VCR.use_cassette('travel_pay/200_claims', match_requests_on: [:method, :path]) do
          get '/travel_pay/claims', params: nil, headers: { 'Authorization' => 'Bearer vagov_token' }
          expect(response).to have_http_status(:ok)
          claim_ids = JSON.parse(response.body)["data"].map { |claim| claim["id"] }
    ...
    context 'unsuccessful response from API' do
      it 'responds with a 404 if the API endpoint is not found' do
        VCR.use_cassette('travel_pay/404_claims', match_requests_on: [:method, :path]) do
          get '/travel_pay/claims', params: nil, headers: { 'Authorization' => 'Bearer vagov_token' }

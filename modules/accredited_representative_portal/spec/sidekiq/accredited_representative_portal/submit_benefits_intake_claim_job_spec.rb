...
RSpec.describe AccreditedRepresentativePortal::SubmitBenefitsIntakeClaimJob, :uploader_helpers do
  ...
  describe '#perform' do
    ...
    before do
      ...
      allow(lighthouse_service).to receive_messages(uuid:, location:, upload_doc: response)
      allow(intake_service).to receive(:valid_document?).and_return(pdf_path)
      allow(monitor).to receive(:track_submission_success)

      allow(job).to receive_messages(
        init: nil,
        process_record: pdf_path,
        create_form_submission_attempt: nil,
        send_confirmation_email: nil,
        cleanup_file_paths: nil
      )
    end
    ...
    it 'performs' do
      ...
      expected_metadata = {
        ...
        zipCode: '12345',
        source: 'AccreditedRepresentativePortal::SavedClaim::BenefitsIntake::DependencyClaim va.gov',
        docType: '21-686c',
        businessLine: 'CMP'
      }.to_json
    ...
    context 'submission has additional documentation' do
      ...
      it 'stamps the footer of the additional docs' do
        ...
        allow(PDFUtilities::DatestampPdf).to receive(:new)
          .with('/tmp/test.pdf').and_return(datestamp_pdf_double)
    ...
    context 'when email sending fails' do
      ...
      it 'logs the error but does not reraise' do
        ...
        allow(notification).to receive(:deliver)
          .with(:confirmation).and_raise(email_error)

        expect(monitor).to receive(:track_send_email_failure).with(
          claim,
          intake_service,
          user_account_uuid,
          'confirmation',

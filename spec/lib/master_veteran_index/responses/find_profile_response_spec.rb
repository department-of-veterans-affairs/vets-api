...
describe MasterVeteranIndex::Responses::FindProfileResponse do
  let(:faraday_response) { instance_double('Faraday::Response') }
  let(:body) { Ox.parse(File.read('spec/support/mvi/find_candidate_response.xml')) }
  let(:ok_response) { MasterVeteranIndex::Responses::FindProfileResponse.with_parsed_response(faraday_response) }
  let(:error_response) { MasterVeteranIndex::Responses::FindProfileResponse.with_server_error }
  let(:not_found_response) { MasterVeteranIndex::Responses::FindProfileResponse.with_not_found }
  ...
  describe '.with_server_error' do
    ...
    it 'optionally sets #error to the passed exception', :aggregate_failures do
      response  = MasterVeteranIndex::Responses::FindProfileResponse.with_server_error(server_error_exception)
  ...
  describe '.with_not_found' do
    ...
    it 'optionally sets #error to the passed exception', :aggregate_failures do
      response  = MasterVeteranIndex::Responses::FindProfileResponse.with_not_found(not_found_exception)
      exception = response.error.errors.first

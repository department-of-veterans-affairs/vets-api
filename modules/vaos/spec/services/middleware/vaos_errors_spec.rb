...
describe VAOS::Middleware::Response::Errors do
  ...
  describe 'anonymize_icn' do
    it 'returns nil if the URI is nil' do
      expect(subject.send(:anonymize_icn, nil)).to be_nil
    end

    it 'returns the original URI if the URI does not contain an ICN' do
      uri = URI.parse('http://example.com')
      expect(subject.send(:anonymize_icn, uri)).to be(uri)
    end
    ...
    it 'returns a URI with the ICN anonymized' do
      ...
      expect(subject.send(:anonymize_icn, uri)).to eql(anon_uri)
    end
  end
end

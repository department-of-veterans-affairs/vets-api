...
describe Identity::Parsers::GCIds do
  ...
  describe '#parse_xml_gcids' do
    ...
    let(:expected_id_hash) do
      {
        icn: expected_icn,
        sec_id: expected_sec_id,
        mhv_ids: expected_mhv_ids,
        active_mhv_ids: expected_active_mhv_ids,
        edipi: expected_edipi,
        vba_corp_id: expected_vba_corp_id,
        idme_id: expected_idme_id,
        vha_facility_ids: expected_vha_facility_ids,
        cerner_facility_ids: expected_cerner_facility_ids,
        cerner_id: expected_cerner_id,
        birls_ids: expected_birls_ids,
        vet360_id: expected_vet360_id,
        icn_with_aaid: expected_icn_with_aaid,
        birls_id: expected_birls_id
      }
    end
    ...
    context 'when input ids is an empty array' do
      ...
      let(:expected_sec_id) { nil }
      let(:expected_mhv_ids) { nil }
      let(:expected_active_mhv_ids) { nil }
      let(:expected_edipi) { nil }
      let(:expected_vba_corp_id) { nil }
      let(:expected_idme_id) { nil }
      let(:expected_vha_facility_ids) { nil }
      let(:expected_cerner_facility_ids) { nil }
      let(:expected_cerner_id) { nil }
      let(:expected_birls_ids) { [] }
      let(:expected_vet360_id) { nil }
      let(:expected_icn_with_aaid) { nil }
      let(:expected_birls_id) { nil }

      it 'returns a hash with nil ids' do
        expect(subject).to eq expected_id_hash
      end
    end

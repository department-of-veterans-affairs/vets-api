...
RSpec.describe BEP::DependentV2Service do
  ...
  let(:service) { BEP::DependentV2Service.new(user) }
  ...
  describe '#submit_686c_form' do
    ...
    context 'enqueues SubmitForm686cJob' do
      it 'fires jobs correctly' do
        VCR.use_cassette('bep/dependent_service/submit_686c_form') do
          expect(BEP::SubmitForm686cV2Job).to receive(:perform_async).with(
            user.uuid, claim.id,
            encrypted_vet_info
          )
          service.submit_686c_form(claim)
    ...
    context 'BEP person is found by participant id or ssn' do
      ...
      it 'submits call to find person by ptcpnt id and logs that the pid is present' do
        ...
        expect(monitor).to receive(:track_event).with(
          'info',
          'BEP::DependentV2Service#get_form_hash_686c found bep_person by PID',
          'bep.dependent_service.find_by_participant_id'
        )
      ...
      it 'submits call to find person by ssn after ptcpnt returns nil and logs that the ssn was used' do
        allow_any_instance_of(BEP::PersonWebService).to receive(:find_by_ssn).and_return({ file_nbr: '796043735' })
        expect_any_instance_of(BEP::PersonWebService).to receive(:find_person_by_ptcpnt_id).and_return(nil)
        expect(monitor).to receive(:track_event).with(
          'info',
          'BEP::DependentV2Service#get_form_hash_686c found bep_person by ssn',
          'bep.dependent_service.find_by_ssn'
        )
    ...
    context 'va_profile_email returns error' do
      ...
      it 'still submits a PDF, enqueues the SubmitForm686cJob with the form email, and tracks the error' do
        ...
        expect(monitor).to receive(:track_event).with(
          'warn', 'BEP::DependentV2Service#get_user_email failed to get va_profile_email',
          'bep.dependent_service.get_va_profile_email.failure', { error: '404 person not found' }
        )
        ...
        VCR.use_cassette('bep/dependent_service/submit_686c_form') do
          ...
          no_email = BEP::DependentV2Service.new(user)
    ...
    context 'on error' do
      ...
      it 'submits to backup job on pdf submission errors' do
        VCR.use_cassette('bep/dependent_service/submit_686c_form') do
          ...
          expect(BEP::SubmitForm686cV2Job).not_to receive(:perform_async)
      ...
      it 'in case of other errors it logs the exception and raises a custom error' do
        VCR.use_cassette('bep/dependent_service/submit_686c_form') do
          allow(BEP::SubmitForm686cV2Job).to receive(:perform_async).and_raise(StandardError,
                                                                               'Test error')
          expect do
            service.submit_686c_form(claim)
      ...
      context 'BEP throws an error - 502' do
        let(:monitor) { instance_double(Dependents::Monitor) }
        ...
        it 'still submits a PDF and enqueues the SubmitForm686cJob' do
          expect_any_instance_of(BEP::PersonWebService)
          ...
          expect(monitor).to receive(:track_event).with(
            ...
            'bep.dependent_service.get_form_hash.failure',
            { error: 'Could not retrieve file number from BEP' }
          )
        ...
        it 'increments StatsD for certain errors - 302,500,502,504' do
          VCR.use_cassette('bep/dependent_service/submit_686c_form') do
            ...
            expect(StatsD)
              ...
              .with(
                'bep.dependent_service.non_validation_error.302',
  ...
  describe '#get_dependents' do
    ...
    it 'returns a valid response when empty array' do
      VCR.use_cassette('bep/dependent_service/get_dependents') do
        expect_any_instance_of(BEP::ClaimantWebService).to receive(:find_dependents_by_participant_id)
          .with(user.participant_id, user.ssn).and_return([])

        response = BEP::DependentV2Service.new(user).get_dependents
  ...
  describe '#submit_674_form' do
    ...
    context 'enqueues SubmitForm674Job' do
      it 'fires jobs correctly' do
        VCR.use_cassette('bep/dependent_service/submit_686c_form') do
          expect(BEP::SubmitForm674V2Job).to receive(:perform_async).with(
            user.uuid, claim.id,
            encrypted_vet_info
          )
          service.submit_686c_form(claim)
  ...
  describe '#submit_pdf_job' do
    ...
    it 'submits evidence pdf via claims evidence uploader' do
      ...
      expect(monitor).to receive(:track_event).with(
        'info', 'BEP::DependentV2Service#submit_pdf_job called to begin ClaimsEvidenceApi::Uploader',
      ...
      expect(monitor).to receive(:track_event).with(
        'info', "BEP::DependentV2Service claims evidence upload of 686C-674-V2 claim_id #{claim.id}",
        "#{stats_key}.claims_evidence.upload", tags: ['form_id:686C-674-V2']
      )
      expect(monitor).to receive(:track_event).with(
        'info', "BEP::DependentV2Service claims evidence upload of 21-674-V2 claim_id #{claim.id}",
        "#{stats_key}.claims_evidence.upload", tags: ['form_id:21-674-V2']
      )
      expect(monitor).to receive(:track_event).with('info', 'BEP::DependentV2Service#submit_pdf_job completed',

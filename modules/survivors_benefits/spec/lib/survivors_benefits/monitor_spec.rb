...
RSpec.describe SurvivorsBenefits::Monitor, skip: 'TODO after schema built' do
  ...
  let(:message_prefix) { "#{described_class} #{SurvivorsBenefits::FORM_ID}" }

  describe '#service_name' do
    it 'returns expected name' do
      expect(monitor.send(:service_name)).to eq('survivors-benefits')
    end
  end
    ...
    describe '#track_submission_exhaustion' do
      context 'with a claim parameter' do
        it 'logs sidekiq job exhaustion' do
          notification = double(SurvivorsBenefits::NotificationEmail)
          ...
          expect(SurvivorsBenefits::NotificationEmail).to receive(:new).with(claim.id).and_return notification
      ...
      context 'without a claim parameter' do
        it 'logs sidekiq job exhaustion' do
          ...
          expect(SurvivorsBenefits::NotificationEmail).not_to receive(:new)

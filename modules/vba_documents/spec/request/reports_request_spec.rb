  ...
  describe '#create /v0/uploads/report' do
    ...
    context 'with in-flight submissions' do
      ...
      it 'should return status of a single upload submissions' do
        ...
        post '/services/vba_documents/v0/uploads/report', uuids: params
    ...
    context 'without in-flight submissions' do
      ...
      it 'should omit results for non-existent submission' do
        post '/services/vba_documents/v0/uploads/report', uuids: ['fake-1234']
        expect(response).to have_http_status(:ok)
        json = JSON.parse(response.body)
        expect(json['data']).to be_an(Array)
    ...
    context 'with invalid parameters' do
      it 'should return error if no uuids parameter' do
        post '/services/vba_documents/v0/uploads/report', foo: 'bar'
        expect(response).to have_http_status(:bad_request)
      end

      it 'should return error if uuids parameter not a list' do
        post '/services/vba_documents/v0/uploads/report', uuids: 'bar'
        expect(response).to have_http_status(:bad_request)
      end

      it 'should return error if uuids parameter has too many elements' do

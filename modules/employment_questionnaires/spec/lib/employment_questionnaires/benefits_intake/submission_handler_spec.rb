...
RSpec.describe EmploymentQuestionnaires::BenefitsIntake::SubmissionHandler do
  let(:handler) { EmploymentQuestionnaires::BenefitsIntake::SubmissionHandler }
  let(:claim) { double(form_id: 'TEST', id: 23) }
  let(:monitor) { double(EmploymentQuestionnaires::Monitor) }
  let(:notification) { double(EmploymentQuestionnaires::NotificationEmail) }
  ...
  before do
    ...
    allow(EmploymentQuestionnaires::NotificationEmail).to receive(:new).with(claim.id).and_return notification
  end

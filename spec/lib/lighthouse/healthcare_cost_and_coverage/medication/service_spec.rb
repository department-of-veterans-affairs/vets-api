# frozen_string_literal: true

require 'rails_helper'
require 'lighthouse/healthcare_cost_and_coverage/medication/service'

RSpec.describe Lighthouse::HealthcareCostAndCoverage::Medication::Service do
  let(:icn) { '43000199' }
  let(:service) { described_class.new(icn) }
  let(:medication_id) { '4-1abOXvmAJfxtQJ' }
  let(:response_body) { { 'resourceType' => 'Medication', 'identifier' => [{ 'id' => '4003210' }] } }
  let(:faraday_response) { double('Faraday::Response', body: response_body) }

  describe '#initialize' do
    it 'initializes with icn' do
      expect(service).to be_a(described_class)
    end

    it 'raises if icn is nil' do
      expect { described_class.new(nil) }.to raise_error(ArgumentError, 'no ICN passed in for HCCC request')
    end

    it 'raises if icn is blank' do
      expect { described_class.new('') }.to raise_error(ArgumentError, 'no ICN passed in for HCCC request')
    end
  end

  describe '#list' do
    before do
      allow(service.send(:config)).to receive(:get).and_return(faraday_response)
    end

    it 'requires id parameter' do
      expect { service.list }.to raise_error(ArgumentError, /missing keyword/)
    end

    it 'returns a FHIR resource hash' do
      result = service.list(id: medication_id)
      expect(result).to eq(response_body)
    end

    it 'queries with _id parameter' do
      expect(service.send(:config)).to receive(:get).with(
        'r4/Medication',
        params: { _id: medication_id },
        icn:
      ).and_return(faraday_response)

      service.list(id: medication_id)
    end

    it 'passes through extra query params' do
      expect(service.send(:config)).to receive(:get).with(
        'r4/Medication',
        params: { _id: medication_id, _count: 50, page: 2 },
        icn:
      ).and_return(faraday_response)

      service.list(id: medication_id, _count: 50, page: 2)
    end

    context 'when Faraday::TimeoutError is raised' do
      before do
        allow(service.send(:config)).to receive(:get).and_raise(Faraday::TimeoutError.new)
      end

      it 'raises Common::Exceptions::Timeout' do
        expect { service.list(id: medication_id) }.to raise_error(Common::Exceptions::Timeout)
      end
    end

    context 'when Faraday::ClientError is raised' do
      before do
        allow(service.send(:config)).to receive(:get).and_raise(Faraday::ClientError.new('fail'))
        allow(Lighthouse::ServiceException).to receive(:send_error).and_return(:error_envelope)
      end

      it 'calls handle_error and returns error envelope' do
        expect(service.list(id: medication_id)).to eq(:error_envelope)
      end
    end
  end
end

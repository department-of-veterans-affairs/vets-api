...
class ScannedUploadPurgeJob
  ...
  def purge_old_submissions(stats)
    ...
    loop do
      form_submissions = fetch_eligible_submissions(cutoff_date, attempted_ids)
      break if form_submissions.empty?

      attempted_ids += form_submissions.map(&:id)
      form_submissions.each { |fs| purge_form_submission(fs, stats) }
    end
  end

  def fetch_eligible_submissions(cutoff_date, exclude_ids = [])
  ...
  def purge_form_submission(form_submission, stats)
    vbms_attempt = form_submission.form_submission_attempts.find_by(aasm_state: 'vbms')

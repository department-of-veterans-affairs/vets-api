...
RSpec.describe EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob, :uploader_helpers,
  ...
  describe '#perform' do
    ...
    before do
      ...
      allow(EmploymentQuestionairres::SavedClaim).to receive(:find).and_return(claim)
    ...
    context 'with employment_questionairres_form_enabled flipper' do
      before do
        allow(UserAccount).to receive(:find).and_return(double('user_account'))
      end

      it 'processes claim when flipper is enabled' do
        allow(Flipper).to receive(:enabled?).with(:employment_questionairres_form_enabled).and_return(true)
        allow(job).to receive(:process_document).and_return(pdf_path)

        expect(EmploymentQuestionairres::SavedClaim).to receive(:find).and_return(claim)
      ...
      it 'returns early when flipper is disabled' do
        allow(Flipper).to receive(:enabled?).with(:employment_questionairres_form_enabled).and_return(false)

        expect(EmploymentQuestionairres::SavedClaim).not_to receive(:find)
    ...
    it 'is unable to find saved_claim_id' do
      allow(EmploymentQuestionairres::SavedClaim).to receive(:find).and_return(nil)
      ...
      expect { job.perform(claim.id, :user_account_uuid) }.to raise_error(
        EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob::EmploymentQuestionairresBenefitIntakeError,
        "Unable to find EmploymentQuestionairres::SavedClaim #{claim.id}"
      )
    end
    # perform
  end
  ...
  describe '#send_submitted_email' do
    ...
    before do
      ...
      allow(EmploymentQuestionairres::NotificationEmail).to receive(:new).and_return(notification)
    ...
    it 'errors and logs but does not reraise' do
      expect(EmploymentQuestionairres::NotificationEmail).to receive(:new).with(claim.id)
  ...
  describe 'sidekiq_retries_exhausted block' do
    let(:exhaustion_msg) do
      { 'args' => [], 'class' => 'EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob',
    ...
    context 'when retries are exhausted' do
      it 'logs a distrinct error when no claim_id provided' do
        EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob.within_sidekiq_retries_exhausted_block do
      ...
      it 'logs a distrinct error when only claim_id provided' do
        EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob
          .within_sidekiq_retries_exhausted_block({ 'args' => [claim.id] }) do
          allow(EmploymentQuestionairres::SavedClaim).to receive(:find).and_return(claim)
          expect(EmploymentQuestionairres::SavedClaim).to receive(:find).with(claim.id)
      ...
      it 'logs a distrinct error when claim is not found' do
        EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob
          ...
          expect(EmploymentQuestionairres::SavedClaim).to receive(:find).with(claim.id - 1)

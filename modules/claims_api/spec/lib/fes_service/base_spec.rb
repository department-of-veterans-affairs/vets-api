...
describe ClaimsApi::FesService::Base do
  ...
  before do
    allow(Settings.claims_api.fes).to receive(:host).and_return('https://api.va.gov')
    allow(Settings.claims_api.fes.auth.ccg).to receive_messages(
      client_id: 'test_client',
      aud_claim_url: 'https://api.va.gov/oauth2/token',
      rsa_key: 'fake_rsa_key'
    )
  ...
  describe '#validate' do
    context 'successful validation' do
      before do
        stub_request(:post, 'https://api.va.gov/form526-establishment-service/v1/validate')
          .with(
  ...
  describe '#submit' do
    context 'successful submission' do
      before do
        stub_request(:post, 'https://api.va.gov/form526-establishment-service/v1/submit')
          .with(
            headers: { 'Authorization' => 'Bearer fake_token' },
            body: form_data.to_json
          )

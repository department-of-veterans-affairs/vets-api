...
module EmailVerificationErrorHandler
  ...
  # Log email verification service errors with context
  ...
  # Log email verification rate limit errors
  def log_verification_rate_limit_error(operation, error)
    ...
    begin
      log_data[:rate_limit_info] = get_email_verification_rate_limit_info if respond_to?(:get_email_verification_rate_limit_info)
    rescue => e
  ...
  # Log unexpected errors in email verification
  def log_unexpected_verification_error(operation, error)
    log_data = email_verification_log_data.merge(
      error: error.message,
      error_class: error.class.name,
      backtrace: error.backtrace&.first(10) # Limit backtrace for email verification
    )

    Rails.logger.error("Email verification unexpected error: #{operation}", log_data)
  end

  # Render email verification service unavailable error
  ...
  # Render internal server error for email verification
  ...
  # Email verification specific logging data

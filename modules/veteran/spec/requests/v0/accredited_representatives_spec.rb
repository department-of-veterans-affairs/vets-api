  ...
  context 'when find a rep is enabled' do
    ...
    context 'when searching for an organization' do
      before do
        ...
        create(:organization, poa: '234', long: -77.436649, lat: 39.101481, location: 'POINT(-77.436649 39.101481)',
                              name: 'Missouri Veterans Commission') # ~25 miles from Washington, D.C.

        create(:organization, poa: '345', long: -76.609383, lat: 39.299236, location: 'POINT(-76.609383 39.299236)',
                              name: 'Alabama Department of Veterans Affairs') # ~35 miles from Washington, D.C.

        create(:organization, poa: '456', long: -77.466316, lat: 38.309875, location: 'POINT(-77.466316 38.309875)',
                              name: 'Washington Department of Veterans Affairs') # ~47 miles from Washington, D.C.

        create(:organization, poa: '789', long: -76.3483, lat: 39.5359, location: 'POINT(-76.3483 39.5359)',
                              name: 'Virginia Department of Veterans Services') # ~57 miles from Washington, D.C.
      end

      it 'does not include organizations outside of max distance' do
        get '/services/veteran/v0/accredited_representatives',
            params: { type: 'organization', lat: 38.9072, long: -77.0369 }

        parsed_response = JSON.parse(response.body)

        expect(parsed_response['data'].pluck('id')).not_to include('789')
      end
      ...
      it 'can sort by name_asc' do
        ...
        expect(parsed_response['data'].pluck('id')).to eq(%w[345 123 234 456])
      end

      it 'can sort by name_desc' do
        get '/services/veteran/v0/accredited_representatives',
            params: { type: 'organization', lat: 38.9072, long: -77.0369, sort: 'name_desc' }

        parsed_response = JSON.parse(response.body)

        expect(parsed_response['data'].pluck('id')).to eq(%w[456 234 123 345])
      end

      it 'can sort fuzzy search on name' do
        get '/services/veteran/v0/accredited_representatives',
            params: { type: 'organization', lat: 38.9072, long: -77.0369, name: 'Bob Law' }
      ...
      it 'serializes with the correct model and distance' do
        ...
        expect(parsed_response['data'][0]['attributes']['name']).to eq('Bob Law')
        expect(parsed_response['data'][0]['attributes']['distance']).to be_within(0.05).of(6.0292)
      end

      it 'paginates' do
        get '/services/veteran/v0/accredited_representatives',
            params: { type: 'organization', lat: 38.9072, long: -77.0369, page: 1, per_page: 2 }
    ...
    context 'when searching for a representative' do
      ...
      it 'does not include representatives of a different type' do
        get '/services/veteran/v0/accredited_representatives',
            params: { type: 'claim_agents', lat: 38.9072, long: -77.0369 }

        parsed_response = JSON.parse(response.body)

        expect(parsed_response['data'].pluck('id')).to eq(['999'])
      end

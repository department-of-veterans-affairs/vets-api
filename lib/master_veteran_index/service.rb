...
require_relative 'messages/add_person_message'
...
# Wrapper for the Master Veteran Index Service. vets.gov has access
# to three MasterVeteranIndex endpoints:
...
module MasterVeteranIndex
  class Service < Common::Client::Base
    ...
    # The MasterVeteranIndex Service SOAP operations vets.gov has access to
    ...
    # @return [MasterVeteranIndex::Configuration] the configuration for this service
    ...
    def add_person(user_identity)
      with_monitoring do
        measure_info(user_identity) do
          ...
          MasterVeteranIndex::Responses::AddPersonResponse.with_parsed_response(raw_response)
        end
      end
      ...
      mvi_add_exception_response_for('MVI_504', e)
    rescue MasterVeteranIndex::Errors::Base => e
    ...
    # Given a user queries MasterVeteranIndex and returns their VA profile.
    #
    # @param user [UserIdentity] the user to query MasterVeteranIndex for
    # @return [MasterVeteranIndex::Responses::FindProfileResponse] the parsed response from MasterVeteranIndex.
    # rubocop:disable Metrics/MethodLength
    def find_profile(user_identity)
      with_monitoring do
        measure_info(user_identity) do
          ...
          MasterVeteranIndex::Responses::FindProfileResponse.with_parsed_response(raw_response)
        end
      end
    rescue Breakers::OutageException => e
      Raven.extra_context(breakers_error_message: e.message)
      ...
      mvi_error_handler(user_identity, e)
      if e.is_a?(MasterVeteranIndex::Errors::RecordNotFound)
    ...
    def measure_info(user_identity)
      Rails.logger.measure_info('Performed MasterVeteranIndex Query', payload: logging_context(user_identity)) { yield }
    end
    ...
    def mvi_add_exception_response_for(key, error)
      ...
      MasterVeteranIndex::Responses::AddPersonResponse.with_server_error(exception)
    end
    ...
    def mvi_profile_exception_response_for(key, error, type: SERVER_ERROR)
      ...
      if type == SERVER_ERROR
        MasterVeteranIndex::Responses::FindProfileResponse.with_server_error(exception)
      else
        MasterVeteranIndex::Responses::FindProfileResponse.with_not_found(exception)
      end
    end
    ...
    def mvi_error_handler(user_identity, e)
      case e
      when MasterVeteranIndex::Errors::DuplicateRecords
        log_console_and_sentry('MVI Duplicate Record', :warn)
      when MasterVeteranIndex::Errors::RecordNotFound
        log_console_and_sentry('MVI Record Not Found')
      when MasterVeteranIndex::Errors::InvalidRequestError
      ...
      when MasterVeteranIndex::Errors::FailedRequestError
    ...
    def create_add_message(user)
      ...
      MasterVeteranIndex::Messages::AddPersonMessage.new(user).to_xml if user.icn_with_aaid.present?
    end
    ...
    def message_icn(user)
      MasterVeteranIndex::Messages::FindProfileMessageICN.new(user.mhv_icn).to_xml
    end

    def message_edipi(user)
      MasterVeteranIndex::Messages::FindProfileMessageEdipi.new(user.dslogon_edipi).to_xml
    end
    ...
    def message_user_attributes(user)
      ...
      MasterVeteranIndex::Messages::FindProfileMessage.new(profile).to_xml
    end
  end
end

...
module EventBusGateway
  class VANotifySmsStatusCallback
    ...
    STATSD_METRIC_PREFIX = 'event_bus_gateway.va_notify_sms_status_callback'
    ...
    def self.call(notification)
      ...
      if notification.status == 'temporary-failure' && Flipper.enabled?(:event_bus_gateway_retry_sms)
        retry_sms(notification)
      end
    end

    def self.retry_sms(notification)
      ebg_noti = find_notification_by_va_notify_id(notification.notification_id)
      return handle_exhausted_retries(notification, ebg_noti) if ebg_noti.attempts >= Constants::MAX_SMS_ATTEMPTS
    ...
    def self.schedule_retry_job(ebg_noti)
      ...
      EventBusGateway::LetterReadyRetrySmsJob.perform_in(
        1.hour,
    ...
    def self.add_exhausted_retry_log(_notification, ebg_notification)
      context = {
        ...
        max_attempts: Constants::MAX_SMS_ATTEMPTS
      }

      Rails.logger.error('EventBusGateway sms retries exhausted', context)
    end

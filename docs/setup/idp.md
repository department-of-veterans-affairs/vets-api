...
1. Constructor args (`base_url`, `timeout`)
2. `Settings.cave.idp.base_url` / `Settings.cave.idp.timeout`
3. `IDP_API_BASE_URL` / `IDP_API_TIMEOUT`
...
1. Add/update `IDP_API_BASE_URL` and `IDP_API_TIMEOUT` in production deployment config.

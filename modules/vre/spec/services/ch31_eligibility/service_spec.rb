...
RSpec.describe VRE::Ch31Eligibility::Service do
  ...
  describe '#get_details' do
    ...
    let(:response) { instance_double(VRE::Ch31Eligibility::Response) }
    let(:url) { "#{Settings.res.base_url}/suite/webapi/chapter31-eligibility-details-search" }
    ...
    context 'when successful' do
      it 'sends payload with icn to RES' do
        ...
        allow(VRE::Ch31Eligibility::Response).to receive(:new)
          .with(raw_response.status, raw_response)
          .and_return(response)
        service.get_details
        expect(service).to have_received(:perform).with(*request_params)
        expect(VRE::Ch31Eligibility::Response).to have_received(:new)
          .with(raw_response.status, raw_response)
      end
    end
    ...
    context 'when RES service unavailable' do
      let(:key) { 'RES_CH31_ELIGIBILITY_500' }

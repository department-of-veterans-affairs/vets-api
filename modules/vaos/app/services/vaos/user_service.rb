...
module VAOS
  class UserService < VAOS::BaseService
    def session(user)
      cached = cached_by_user_account_uuid(user.user_account_uuid)
    ...
    def update_session_token(user_account_uuid)
      cached = cached_by_user_account_uuid(user_account_uuid)
      if cached
        url = '/users/v2/session/jwts'
        response = perform(:get, url, nil, refresh_headers(user_account_uuid))
        ...
        save_session!(user_account_uuid, new_token)
        new_token
      else
        Rails.logger.warn('VAOS no session to update', user_account_uuid:)
      end
    rescue => e
      Rails.logger.error('VAOS session update failed', { user_account_uuid:, error: e.message })
      raise e
    end

    private

    def cached_by_user_account_uuid(user_account_uuid)
      SessionStore.find(user_account_uuid)
    end
    ...
    def lock_session_creation(user_account_uuid)
      redis_session_lock.set(user_account_uuid, 1)
      redis_session_lock.expire(user_account_uuid, REDIS_CONFIG[:va_mobile_session_refresh_lock][:each_ttl])
    end

    def save_session!(user_account_uuid, token)
    ...
    def refresh_headers(user_account_uuid)
      {
        'Referer' => referrer,
        'X-VAMF-JWT' => cached_by_user_account_uuid(user_account_uuid).token,

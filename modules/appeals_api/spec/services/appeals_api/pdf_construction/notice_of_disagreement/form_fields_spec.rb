...
describe AppealsApi::PdfConstruction::NoticeOfDisagreement::FormFields do
  ...
  describe '#veteran_ssn' do
    it { expect(form_fields.veteran_ssn).to eq('F[0].Page_1[0].VeteransSocialSecurityNumber_FirstThreeNumbers[0]') }
  end
  ...
  describe '#evidence_submission?' do
    it { expect(form_fields.evidence_submission?).to eq('F[0].Page_1[0].DecisionReviewOfficer_DROReviewProcess[2]') }
  end

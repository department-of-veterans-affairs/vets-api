  ...
  describe '.to_xml' do
    context 'with an edipi' do
      ...
      it 'includes the edipi' do
        expect(xml).to eq_text_at_path('env:Body/v12:edipiORicn/v13:edipiORicnValue[0]', edipi)
      end

      it 'says it includes the EDIPI' do
        expect(xml).to eq_text_at_path('env:Body/v12:edipiORicn/v13:inputType[0]', 'EDIPI')
      end
    end

    context 'with an icn' do
      let(:icn) { Faker::Number.number(10) }
      let(:xml) do
        EMIS::Messages::EdipiOrIcnMessage.new(icn: icn).to_xml
      end

      it 'includes the icn' do
        expect(xml).to eq_text_at_path('env:Body/v12:edipiORicn/v13:edipiORicnValue[0]', icn)
      end

      it 'says it includes the ICN' do
        expect(xml).to eq_text_at_path('env:Body/v12:edipiORicn/v13:inputType[0]', 'ICN')
      end
    end
    ...
    context 'bad arguments' do
      ...
      it 'should throw an argument error with both identifiers' do
        expect do
          EMIS::Messages::EdipiOrIcnMessage.new(edipi: 1234, icn: 5678)
        end.to raise_error(ArgumentError, 'must include either an EDIPI or ICN, but not both')
      end
    end
  end

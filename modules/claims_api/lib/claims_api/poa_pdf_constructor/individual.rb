...
module ClaimsApi
  module PoaPdfConstructor
    class Individual < ClaimsApi::PoaPdfConstructor::Base
      protected
      ...
      def page1_options(data)
        base_form = 'form1[0].#subform[0]'
        {
          ...
          "#{base_form}.Veterans_MailingAddress_NumberAndStreet[0]": retrieve_street_address(data),
          "#{base_form}.MailingAddress_ApartmentOrUnitNumber[1]": retrieve_unit_or_apartment(data),
          "#{base_form}.MailingAddress_City[1]": data.dig('veteran', 'address', 'city'),
          "#{base_form}.MailingAddress_StateOrProvince[1]": retrieve_state(data),
          "#{base_form}.MailingAddress_Country[1]": data.dig('veteran', 'address', 'country'),
          "#{base_form}.MailingAddress_ZIPOrPostalCode_FirstFiveNumbers[1]": retrieve_zip_code_first_five(data),
          "#{base_form}.MailingAddress_ZIPOrPostalCode_ZIPOrPostalCode_LastFourNumbers[1]": retrieve_zip_code_last_four(data),
          ...
          "#{base_form}.MailingAddress_NumberAndStreet[0]": retrieve_street_address(data, 'claimant'),
          "#{base_form}.MailingAddress_ApartmentOrUnitNumber[0]": retrieve_unit_or_apartment(data, 'claimant'),
          "#{base_form}.MailingAddress_City[0]": data.dig('claimant', 'address', 'city'),
          "#{base_form}.MailingAddress_StateOrProvince[0]": retrieve_state(data, 'claimant'),
          "#{base_form}.MailingAddress_Country[0]": data.dig('claimant', 'address', 'country'),
          "#{base_form}.MailingAddress_ZIPOrPostalCode_FirstFiveNumbers[0]": retrieve_zip_code_first_five(data, 'claimant'),
          "#{base_form}.MailingAddress_ZIPOrPostalCode_ZIPOrPostalCode_LastFourNumbers[0]": retrieve_zip_code_last_four(data, 'claimant'),

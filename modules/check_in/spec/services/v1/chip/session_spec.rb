...
describe V1::Chip::Session do
  ...
  describe '.build' do
    it 'returns an instance of Session' do
      expect(subject.build).to be_an_instance_of(V1::Chip::Session)
    end
  end
  ...
  describe '#session_from_redis' do
    context 'when cache exists' do
      it 'returns an instance of SessionStore' do
        allow_any_instance_of(V1::Chip::RedisHandler).to receive(:session_id).and_return('my_session_id')

        Rails.cache.write('my_session_id', '12345', namespace: 'check-in-chip-v1-cache')
    ...
    context 'when cache does not exist' do
      it 'returns nil' do
        allow_any_instance_of(V1::Chip::RedisHandler).to receive(:session_id).and_return(nil)
  ...
  describe '#establish_chip_session' do
    let(:mock_token) { double('V1::Chip::Token', access_token: 'new_chip_token', created_at: Time.zone.now) }

    it 'returns token from cache' do
      allow_any_instance_of(V1::Chip::Token).to receive(:fetch).and_return(mock_token)
  ...
  describe '#session_id' do
    it 'returns the formatted session_id' do
      expect(subject.build.session_id).to eq('check_in_chip_v1_2dcdrrn5zc')
    end
  end
end

...
module BGS
  class DependentService < BaseService
    ...
    def submit_686c_form(claim)
      ...
      BGS::SubmitForm686cJob.new.perform(@user.uuid, claim.id, vet_info.to_686c_form_hash) if claim.submittable_686?
      BGS::SubmitForm674Job.new.perform(@user.uuid, claim.id, vet_info.to_686c_form_hash) if claim.submittable_674?

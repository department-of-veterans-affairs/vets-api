...
class DocumentUploadSubmission < ActiveRecord::Base
  include SetGuid
  ...
  def signed_url(guid)
    s3 = Aws::S3::Resource.new(region: Settings.documents.s3.region,
                               ...
                               secret_access_key: Settings.documents.s3.aws_access_key_id)
    obj = s3.bucket(Settings.documents.s3.bucket).object(guid)
    obj.presigned_url(:put, {:content_type => 'multipart/form-data'})
  end
end

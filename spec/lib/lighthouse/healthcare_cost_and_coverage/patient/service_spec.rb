...
RSpec.describe Lighthouse::HealthcareCostAndCoverage::Patient::Service do
  ...
  describe '#read' do
    ...
    it 'raises if id is blank' do
      expect { service.read(nil) }.to raise_error(ArgumentError)
      expect { service.read('') }.to raise_error(ArgumentError)
    end
    ...
    context 'when Faraday::TimeoutError is raised' do
      ...
      it 'calls handle_error and returns error envelope' do
        expect(service.read(icn)).to eq(:error_envelope)
      end
    end

...
describe AppealsApi::PdfConstruction::HigherLevelReview::V2::Structure do
  ...
  describe '#insert_overlaid_pages' do
    it 'returns a temporary overlaid pdf path' do
      ...
      expect(result).to eq("/tmp/HLRv2-#{higher_level_review.id}-overlaid-form-fill-tmp.pdf")
    end
  end
  ...
  describe 'form_title' do
    it 'returns the HLR doc title' do
      expect(described_class.new(higher_level_review).form_title).to eq('200996_v2')
    end
  end
end

  ...
  module Specs
    class SwaggerFormatter < ::RSpec::Core::Formatters::BaseTextFormatter
            ...
            doc[:paths]&.each_pair do |_k, v|
              v.each_pair do |_verb, value|
                ...
                if is_hash && value[:parameters]
                  schema_param = value[:parameters]&.find { |p| ([:body, :formData].include?(p[:in])) && p[:schema] }
                  ...
                  value[:parameters].reject! { |p| [:body, :formData].include?(p[:in]) }
                end
      ...
      def request_examples(value)
        examples = value[:parameters]&.find { |p| ([:body, :formData].include?(p[:in])) && p[:examples] }

...
RSpec.describe ArpInProgressForm, type: :model do
  let(:arp_in_progress_form) { build(:arp_in_progress_form) }
  ...
  describe 'validations' do
    it 'validates presence of form_data' do
      expect_attr_valid(arp_in_progress_form, :form_data)
      arp_in_progress_form.form_data = nil
      expect_attr_invalid(arp_in_progress_form, :form_data, "can't be blank")
    end
  end

  describe '#metadata' do
    it 'adds the form expiration time and id', run_at: '2017-06-01' do
      arp_in_progress_form.save
      expect(arp_in_progress_form.metadata['expiresAt']).to eq(1_501_459_200)
      expect(arp_in_progress_form.metadata['ArpinProgressFormId']).to be_an(Integer)
    end

    context 'skips the expiration_date callback wihen skip_exipry_update is true' do
      it 'adds the form expiration time and id', run_at: '2017-06-01' do
        arp_in_progress_form.skip_exipry_update = true
        arp_in_progress_form.save
        expect(arp_in_progress_form.metadata['expires_at']).not_to eq(1_501_459_200)
      end

      it 'sets skip_exipry_update to true for pension form if expiration date already exists' do
        form = create(:arp_in_progress_form, form_id: '21P-527EZ')
        form.save
    ...
    context 'when the form is 21-526EZ' do
      before { arp_in_progress_form.form_id = '21-526EZ' }

      it 'adds a later form expiration time and id', run_at: '2017-06-01' do
        arp_in_progress_form.save
        expect(arp_in_progress_form.metadata['expiresAt']).to eq(1_527_811_200)
        expect(arp_in_progress_form.metadata['ArpinProgressFormId']).to be_an(Integer)
      end

      it 'adds a later form expiration time when a leap year', run_at: '2020-06-01' do
        arp_in_progress_form.save
        expect(arp_in_progress_form.metadata['expiresAt']).to eq(1_622_505_600)
      end
    end

    context 'when the form is 5655' do
      before { arp_in_progress_form.form_id = '5655' }

      it 'does not update expires_at on save' do
        fsr_form = create(:arp_in_progress_form, form_id: '5655')
        fsr_prev_time = fsr_form.expires_at
        normal_form = create(:arp_in_progress_form)
    ...
    it 'adds the form creation time', run_at: '2023-09-15' do
      arp_in_progress_form.save
      expect(arp_in_progress_form.metadata['createdAt']).to eq(1_694_736_000)
    end
  end
  ...
  describe '#serialize_form_data' do
    ...
    it 'serializes form_data as json' do
      arp_in_progress_form.form_data = form_data
      arp_in_progress_form.save!

      expect(arp_in_progress_form.form_data).to eq(form_data.to_json)
    end
  end

  describe 'scopes' do
    let!(:first_record) do
      create(:arp_in_progress_form, metadata: { submission: { hasAttemptedSubmit: true,
                                                              errors: 'foo',
                                                              errorMessage: 'bar',
                                                              status: 'serverError' } })
    end
    let!(:second_record) do
      create(:arp_in_progress_form, metadata: { submission: { hasAttemptedSubmit: false, status: false } })
    end
    let!(:third_record) do
      create(:arp_in_progress_form, form_id: '5655',
                                    metadata: { submission: { hasAttemptedSubmit: true, status: false } })
    end
  ...
  describe '#expires_after' do
    context 'with in_progress_form_custom_expiration Flipper disabled' do
      context 'when 21-526EZ' do
        before do
          arp_in_progress_form.form_id = '21-526EZ'
        end

        it 'is an ActiveSupport::Duration' do
          Flipper.disable(:in_progress_form_custom_expiration)

          expect(arp_in_progress_form.expires_after).to be_a(ActiveSupport::Duration)
        end

        it 'value is 1 year' do
          Flipper.disable(:in_progress_form_custom_expiration)

          expect(arp_in_progress_form.expires_after).to eq(1.year)
        end
      end

      context 'when unrecognized form' do
        before do
          arp_in_progress_form.form_id = 'abcd-1234'
        end
        ...
        it 'value is 60 days' do
          ...
          expect(arp_in_progress_form.expires_after).to eq(60.days)
        end
      end
    end
    ...
    context 'with in_progress_form_custom_expiration Flipper enabled' do
      before do
        arp_in_progress_form.form_id = 'HC-QSTNR_abc123'
        arp_in_progress_form.form_data = { days_till_expires: '60' }.to_json
      end

      it 'expires in 60 days' do
  ...
  describe '.form_for_user' do
    subject { ArpInProgressForm.form_for_user(form_id, user) }

    let(:form_id) { arp_in_progress_form.form_id }
    ...
    context 'and in progress form exists associated with user uuid' do
      let!(:in_progress_form_user_uuid) { create(:arp_in_progress_form, user_uuid: user.uuid) }

      it 'returns ArpInProgressForms for user with given form id' do
    ...
    context 'and in progress form does not exist associated with user uuid' do
      context 'and in progress form exists associated with user account on user' do
        let!(:in_progress_form_user_account) { create(:arp_in_progress_form, user_account: user.user_account) }
  ...
  describe '.for_user' do
    subject { ArpInProgressForm.for_user(user) }

...
describe VAOS::SystemsService do
    ...
    context 'with express care' do
      it 'includes express care data' do
        VCR.use_cassette('vaos/systems/get_system_facilities_express_care', match_requests_on: %i[method uri]) do
          ...
          expect(response.map { |h| h[:express_times] }).to eq(
            [
              {
                start: '09:17',
                end: '16:45',
  ...
  describe '#get_request_eligibility_criteria' do
    context 'with no site_codes param' do
      it 'returns the full list', :aggregate_failures do
        VCR.use_cassette('vaos/systems/get_request_eligibility_criteria', match_requests_on: %i[method uri]) do
          ...
          expect(first_result.request_settings.first).to eq(
            { id: '502', type_of_care: 'Outpatient Mental Health', patient_history_required: 'No', stop_codes: [{ primary: '502' }], submitted_request_limit: 2, enterprise_submitted_request_limit: 2 }
          )
        end
      end
    end
        ...
        VCR.use_cassette('vaos/systems/get_request_eligibility_criteria_by_id',
          ...
          expect(response.first.to_h).to eq(
            { links: [{ title: 'request-eligibility-criteria',
              ...
              id: '405GC',
              created_date: '2020-07-16T23:27:52Z',
              last_modified_date: '2020-07-16T23:27:52Z',
              request_settings: [{ id: '502',

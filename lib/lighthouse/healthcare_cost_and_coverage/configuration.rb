...
module Lighthouse
  module HealthcareCostAndCoverage
    class Configuration < Common::Client::Configuration::REST
      ...
      ##
      # Base host (e.g., https://sandbox-api.va.gov)
      def base_path(host = nil)
        (host || settings.host).to_s
      end

      ##
      # Full API base (FHIR endpoints live under here)
      def base_api_path(host = nil)
        "#{base_path(host)}/#{API_PATH}"
      end

      ##
      # Service name (for breakers/metrics)
      def service_name
        'HealthcareCostAndCoverage'
      end

      ##
      # GET with auth header (bearer is minted using ICN -> launch)
      def get(path, options = {})
        connection.get(path, options[:params], { 'Authorization' => "Bearer #{access_token(options)}" })
      end

      ##
      # POST with auth header (not commonly needed for read-only, but here for parity)
      def post(path, body, options = {})
        connection.post(path, body, { 'Authorization' => "Bearer #{access_token(options)}" })
      end

      ##
      # Faraday connection configured for FHIR
      def connection
      ...
      # Build auth params for the token request
      # Expects :icn in options; converts to Base64 launch JSON.

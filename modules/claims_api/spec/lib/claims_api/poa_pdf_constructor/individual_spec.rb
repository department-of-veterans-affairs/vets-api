...
describe ClaimsApi::PoaPdfConstructor::Individual do
  let(:temp_v2) { create(:power_of_attorney, :with_full_headers) }
  let(:temp_v1) { create(:power_of_attorney, :with_full_headers) }
  ...
  it 'construct pdf for v1' do
    power_of_attorney = ClaimsApi::PowerOfAttorney.find(temp_v1.id)
  ...
  it 'construct pdf for v2' do
    power_of_attorney = ClaimsApi::PowerOfAttorney.find(temp_v2.id)
    data = power_of_attorney.form_data.deep_merge(
      {
        'veteran' => {
          'firstName' => power_of_attorney.auth_headers['va_eauth_firstName'],

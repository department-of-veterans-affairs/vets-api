...
module Chatbot
  class ReportToCxi
    def report_to_cxi(icn, conversation_id)
      ...
      send_to_cxi dataverse_uri, icn, conversation_id, token
    end
      ...
      response = Net::HTTP.post URI("#{dataverse_uri}/api/data/v9.2//#{cxi_table_prefix}claimsqueries"),
                                {
                                  ...
                                  "#{cxi_table_prefix}icn" => icn,
                                  "#{cxi_table_prefix}conversationid" => conversation_id,
                                  "#{cxi_table_prefix}requestedtimestamp" => Time.current.to_s
                                }.to_json,
                                {

...
RSpec.describe EmploymentQuestionairres::V0::ClaimsController, type: :request do
  let(:monitor) { double('EmploymentQuestionairres::Monitor') }
  let(:user) { create(:user) }

  before do
    sign_in_as(user)
    Flipper.enable(:employment_questionairres_form_enabled)
  ...
  describe '#create' do
    let(:claim) { build(:employment_questionairres_claim) }
    ...
    it 'logs validation errors' do
      ...
      expect(EmploymentQuestionairres::BenefitsIntake::SubmitClaimJob).not_to receive(:perform_async)
  ...
  describe '#show' do
    it 'logs an error if no claim found' do
      ...
      get '/employment_questionairres/v0/claims/:id', params: { id: 'non-existant-saved-claim' }
    ...
    it 'logs an error' do
      ...
      allow(EmploymentQuestionairres::SavedClaim).to receive(:find_by!).and_raise(error)
    ...
    it 'returns a serialized claim' do
      claim = build(:employment_questionairres_claim)
      allow(EmploymentQuestionairres::SavedClaim).to receive(:find_by!).and_return(claim)

      get '/employment_questionairres/v0/claims/:id', params: { id: 'employment_questionairres_claim' }

...
module Eps
  ...
  class AppointmentStatusJob
    ...
    def log_missing_redis_data(appointment_data)
      ...
      StatsD.increment("#{STATSD_PREFIX}.failure", tags: ['appointment_type:community_care'])
    end
    ...
    def handle_appointment_response(response, retry_count)
      if appointment_finished?(response)
        StatsD.increment("#{STATSD_PREFIX}.success", tags: ['appointment_type:community_care'])

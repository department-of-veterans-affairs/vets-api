...
describe MVI::Messages::FindCandidateMessage do
  describe '.build' do
    context 'with first, last, dob, and ssn from auth provider' do
      let(:xml) { MVI::Messages::FindCandidateMessage.build('John', 'Smith', Time.new(1980, 1, 1).utc, '555-44-3333') }
    ...
    context 'a missing argument' do
      it 'should throw an argument error' do
        expect do
          MVI::Messages::FindCandidateMessage.build('John', 'Smith', Time.new(1980, 1, 1).utc)
    ...
    context 'with invalid name args' do
      it 'should throw an argument error' do
        expect do
          MVI::Messages::FindCandidateMessage.build(:John, 5, Time.new(1980, 1, 1).utc, '555-44-3333')
        end.to raise_error(ArgumentError, 'names should be Strings')
      end
    end
    context 'with an invalid ssn' do
      it 'should throw an argument error' do
        expect do
          MVI::Messages::FindCandidateMessage.build('John', 'Smith', Time.new(1980, 1, 1).utc, '555-4-3333')

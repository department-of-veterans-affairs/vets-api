...
describe MasterVeteranIndex::Responses::IdParser do
  describe '#parse' do
    context 'vba_corp_id' do
      ...
      it 'matches correctly on all valid ID statuses (i.e. P and A)' do
        expect(MasterVeteranIndex::Responses::IdParser.new.parse(vba_corp_ids)[:vba_corp_id]).to eq '12345678'
      end
    end
    ...
    context 'icn_with_aaid' do
      ...
      context 'with all valid ICN components' do
        ...
        it 'matches correctly on the MasterVeteranIndex::Responses::ProfileParser::ICN_REGEX' do
...
def ids_in(body)
  original_body = body.locate(MasterVeteranIndex::Responses::ProfileParser::BODY_XPATH)&.first
  subject = original_body.locate(MasterVeteranIndex::Responses::ProfileParser::SUBJECT_XPATH)&.first
  patient = subject.locate(MasterVeteranIndex::Responses::ProfileParser::PATIENT_XPATH)&.first

  patient.locate('id')
end

def correlation_id(extension)
  OpenStruct.new(attributes: { extension: extension, root: MasterVeteranIndex::Responses::IdParser::VA_ROOT_OID })
end
...
def expect_valid_icn_with_aaid_from_parsed_xml(xml_file:, expected_icn_with_aaid:)
  ...
  correlation_ids = MasterVeteranIndex::Responses::IdParser.new.parse(ids_in(body))
...
def expect_valid_icn_with_aaid_to_be_returned(icn_with_aaid_with_id_status, valid_icn_with_aaid)
  ...
  correlation_ids = MasterVeteranIndex::Responses::IdParser.new.parse(ids)

...
RSpec.describe DigitalFormsApi::Configuration do
  ...
  let(:config) { DigitalFormsApi::Configuration.send(:new) }
  let(:settings) do
    OpenStruct.new({
  ...
  before do
    allow(Settings).to receive(:digital_forms_api).and_return(settings)
  end
  ...
  context 'expected constants' do
    it 'returns service_name' do
      expect(config.service_name).to eq('DigitalFormsApi')
    end

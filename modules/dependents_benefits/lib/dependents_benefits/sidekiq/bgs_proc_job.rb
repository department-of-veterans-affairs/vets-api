...
module DependentsBenefits
  module Sidekiq
    ##
    # Submission job for BGS vnp_proc (Veteran Notification Process)
    #
    # Handles the creation of a BGS vnp_proc to group related form submissions
    ...
    class BGSProcJob < DependentSubmissionJob
      ...
      # Handles successful proc creation by triggering child claim submissions
      #
      # Following proc_id and proc_form success, trigger the enqueueing of child claim
      # submissions. If enqueueing fails, sends a backup job.
      #
      # @return [void]
      ...
      #
      # Updates submission attempt record to success status.
      #
      ...
      #
      # Updates submission attempt record with failure status and stores the exception details.
      #
      ...
      # Always returns false for proc jobs
      #
      # We don't yet know which errors are permanent failures for proc jobs.
      # This allows the job to retry on all error types rather than skipping retries
      # for certain errors.
      #
      # @param _error [Exception] The error to check (unused)
      # @return [Boolean] Always false (all errors are considered transient)

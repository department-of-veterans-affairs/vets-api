...
RSpec.describe 'Appeals API SNS upload complete notification', type: :request do
  let(:path) { '/services/appeals/v1/internal/upload_complete' }
  let(:aws_sns_topic) { 'arn:aws:sns:us-west-2:123456789012:MyTopic' }

  # rubocop:disable Style/ExplicitBlockArgument
  def with_sns_topic
    with_settings(Settings.modules_appeals_api.sns, 'topic_arns' => [aws_sns_topic], 'region' => 'us-gov-west-1') do
      yield
    end
  end
  ...
  context 'with a subscriptionconfirmation message type' do
    ...
    let(:headers) do
      {
        ...
        'x-amz-sns-topic-arn' => aws_sns_topic,
        'Content-Length' => '1336',
    ...
    context 'with incorrect arn' do
      it 'responds with a parameter missing error' do
        with_settings(Settings.modules_appeals_api.sns,
  ...
  context 'with a notification message type' do
    let(:upload) { FactoryBot.create(:evidence_submission) }
    ...
    let(:body) do
      {
        ...
        'TopicArn' => aws_sns_topic,
        'Subject' => 'test',
        'Message' => "{\"Records\":[{\"eventVersion\":\"2.0\",\"eventSource\":\"aws:s3\",\"awsRegion\":\"us-east-1\",\"eventTime\":\"1970-01-01T00:00:00.000Z\",\"eventName\":\"ObjectCreated:Put\",\"userIdentity\":{\"principalId\":\"AIDAJDPLRKLG7UEXAMPLE\"},\"requestParameters\":{\"sourceIPAddress\":\"127.0.0.1\"},\"responseElements\":{\"x-amz-request-id\":\"C3D13FE58DE4C810\",\"x-amz-id-2\":\"FMyUVURIY8/IgAtTv8xRjskZQpcIZ9KG4V5Wp6S7S/JRWeUWerMUE5JgHvANOjpD\"},\"s3\":{\"s3SchemaVersion\":\"1.0\",\"configurationId\":\"testConfigRule\",\"bucket\":{\"name\":\"mybucket\",\"ownerIdentity\":{\"principalId\":\"A3NL1KOZZKExample\"},\"arn\":\"arn:aws:s3:::mybucket\"},\"object\":{\"key\":\"#{upload.supportable_id}\",\"size\":1024,\"eTag\":\"d41d8cd98f00b204e9800998ecf8427e\",\"versionId\":\"096fKKXTRTtl3on89fVO.nfljtsv6qko\",\"sequencer\":\"0055AED6DCD90281E5\"}}}]}",
    ...
    context 'verified message' do
      it 'queues a processor working on the uploaded object-key' do
        with_sns_topic do
          ...
          allow(s3_bucket).to receive(:object).with(upload.supportable_id).and_return(s3_object)
          allow(s3_object).to receive(:exists?).and_return(true)
          allow(s3_client).to receive(:bucket).and_return(s3_bucket)
          # expect(VBADocuments::UploadProcessor).to receive(:perform_async).with(upload.supportable_id)
          verifier = double(Aws::SNS::MessageVerifier)
          allow(verifier).to receive(:authentic?).and_return(true)
          allow(Aws::SNS::MessageVerifier).to receive(:new).and_return(verifier)
          post path, params: body, headers: headers

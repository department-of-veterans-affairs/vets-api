...
module SAML
  module UserAttributes
    class MHV < Base
      ...
      MHV_SERIALIZABLE_ATTRIBUTES = %i[mhv_icn mhv_correlation_id mhv_account_type].freeze
      ...
      def loa_current
        PREMIUM_LOAS.include?(mhv_account_type) ? 3 : 1
      end

      # This is "highest attained" via idp
      # if the account_type PREMIUM or IDME = 3,
      def loa_highest
        PREMIUM_LOAS.include?(mhv_account_type) ? 3 : (idme_loa || loa_current)
      end
    end
  end
end

  ...
  module PdfFill
    ...
    class Section10 < Section # rubocop:disable Metrics/ClassLength
        ...
        'careExpenses' => {
          ...
          'recipient' => {
            key_from_iterator: ->(iterator) {
              puts "#{iterator}, #{CARE_CONFIGS.dig(iterator, :recipient_field)}"
              CARE_CONFIGS.dig(iterator, :recipient_field)
            }
          },
          'recipientName' => {
      ...
      def transform_care_expense(entry, config)
        ...
        data['recipientName'] = entry['recipientName'] || entry['recipientOther'] || entry['childName']
      ...
      def format_numeric_field(value, length: 3)
        return nil if value.blank? or value.class != Integer

...
RSpec.describe SOB::V2::Post911GIBillStatusesController, type: :controller do
  routes { SOB::Engine.routes }
  ...
  context 'service is available' do
    it 'returns a 200 success' do
      ...
      VCR.use_cassette('lighthouse/benefits_education/gi_bill_status/200_response') do
        expect(StatsD).to receive(:increment).with("#{SOB::V2::Post911GIBillStatusesController::STATSD_KEY_PREFIX}.total")
    ...
    it 'returns a 404 when vet isn\'t found' do
      VCR.use_cassette('lighthouse/benefits_education/gi_bill_status/404_response') do
        expect(StatsD).to receive(:increment).with("#{SOB::V2::Post911GIBillStatusesController::STATSD_KEY_PREFIX}.fail",
                                                   tags: ['error:404'])
        expect(StatsD).to receive(:increment).with("#{SOB::V2::Post911GIBillStatusesController::STATSD_KEY_PREFIX}.total")
        expect do
          get :show

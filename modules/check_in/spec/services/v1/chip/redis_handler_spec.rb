...
describe V1::Chip::RedisHandler do
  ...
  describe '.build' do
    it 'returns an instance of RedisHandler' do
      expect(subject.build).to be_an_instance_of(V1::Chip::RedisHandler)
    end
  end
  ...
  describe '#save' do
    it 'is true' do
      allow_any_instance_of(subject).to receive(:session_id).and_return('my_session_id')
      allow_any_instance_of(V1::Chip::Token).to receive(:access_token).and_return('my_token')
      allow_any_instance_of(V1::Chip::Token).to receive(:created_at).and_return('my_time')
      allow_any_instance_of(subject).to receive(:token).and_return(V1::Chip::Token.build)

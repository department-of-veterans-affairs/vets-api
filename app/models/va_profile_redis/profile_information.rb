...
module VAProfileRedis
  # Facade for VAProfile::ProfileInformation::Service. The user_serializer delegates
  # to this class through the User model.
  #
  # When a person is requested from the serializer, it returns either a cached
  # response in Redis or from the VAProfile::ProfileInformation::Service.
  #
  class ProfileInformation < Common::RedisStore
    ...
    # The status of the last VAProfile::ProfileInformation::Service response,
    # or not authorized for for users < LOA 3
    #
    # @return [Integer <> String] the status of the last VAProfile::ProfileInformation::Service response
    #
    def status
      return VAProfile::ProfileInformation::PersonResponse::RESPONSE_STATUS[:not_authorized] unless @user.loa3?

      response.status
    end

    # @return [VAProfile::ProfileInformation::PersonResponse] the response returned from
    # the redis cache.  If that is unavailable, it calls the
    # VAProfile::ProfileInformation::Service#get_response("person") endpoint.
    #
    def response
    ...
    # VAProfileRedis::ProfileInformation object with the uuid necessary
    ...
    def response_from_redis_or_service
      return profile_information_service.get_response("person") unless VAProfile::Configuration::SETTINGS.profile_information.cache_enabled

      do_cached_with(key: @user.uuid) do
        profile_information_service.get_response("person")
      end
    end

    def profile_information_service
      @service ||= VAProfile::ProfileInformation::Service.new(@user)
    end
  end
end

# frozen_string_literal: true

require 'rails_helper'

describe PdfFill::Forms::FormHelper do

  let(:including_class) { Class.new { include FormHelper } }

  describe '#split_ssn' do
    it 'should return nil' do
      expect(including_class.split_ssn('')).to eq(nil)
    end

    it 'should split the ssn' do
      expect(including_class.split_ssn('111223333')).to eq('first' => '111', 'second' => '22', 'third' => '3333')
    end

  #   it 'should remove the extra characters and split the ssn' do
  #     expect(described_class.split_ssn('111-22-3333')).to eq('first' => '111', 'second' => '22', 'third' => '3333')
  #   end
  end

  # describe '#extract_va_file_number' do
  #   it 'should strip the leading character' do
  #     expect(described_class.extract_va_file_number('c123456789')).to eq('123456789')
  #   end

  #   it 'should leave the file number unchanged' do
  #     expect(described_class.extract_va_file_number('c12345678')).to eq('c12345678')
  #   end
  # end

  # describe '#extract_middle_i' do
  #   it 'veteran with no name should return nil' do
  #     expect(described_class.extract_middle_i({}, 'veteranFullName')).to eq(nil)
  #   end

  #   it 'veteran name with no middle name should return nil' do
  #     veteran_full_name = {
  #       'veteranFullName' => {
  #         'first' => 'testy',
  #         'last' => 'testerson'
  #       }
  #     }
  #     expect(described_class.extract_middle_i(veteran_full_name, 'veteranFullName')).to eq(
  #       'first' => 'testy',
  #       'last' => 'testerson'
  #     )
  #   end

  #   it 'should extract middle initial when there is a middle name' do
  #     veteran_full_name = {
  #       'veteranFullName' => {
  #         'middle' => 'middle'
  #       }
  #     }
  #     expect(described_class.extract_middle_i(veteran_full_name, 'veteranFullName')).to eq(
  #       'middle' => 'middle',
  #       'middleInitial' => 'm'
  #     )
  #   end
  # end

  # describe '#extract_country' do
  #   it 'should return nil with blank address' do
  #     expect(described_class.extract_country({})).to eq(nil)
  #   end

  #   it 'should return nil with no country' do
  #     address = {
  #       'city' => 'Baltimore'
  #     }
  #     expect(described_class.extract_country(address)).to eq(nil)
  #   end

  #   it 'should return first two digits for country' do
  #     address = {
  #       'country' => 'USA'
  #     }
  #     expect(described_class.extract_country(address)).to eq('US')
  #   end

  #   it 'should return nil for blank country' do
  #     address = {
  #       'country' => ''
  #     }
  #     expect(described_class.extract_country(address)).to eq(nil)
  #   end
  # end

  # describe '#split_postal_code' do
  #   it 'should return nil with blank address' do
  #     expect(described_class.split_postal_code({})).to eq(nil)
  #   end

  #   it 'should return nil with no postal code' do
  #     address = {
  #       'city' => 'Baltimore'
  #     }
  #     expect(described_class.split_postal_code(address)).to eq(nil)
  #   end

  #   it 'should return nil for blank postal code' do
  #     address = {
  #       'postalCode' => ''
  #     }
  #     expect(described_class.split_postal_code(address)).to eq(nil)
  #   end

  #   it 'should split the code correctly with extra characters' do
  #     address = {
  #       'postalCode' => '12345-0000'
  #     }
  #     expect(described_class.split_postal_code(address)).to eq('firstFive' => '12345', 'lastFour' => '0000')
  #   end

  #   it 'should split the code correctly with 9 digits' do
  #     address = {
  #       'postalCode' => '123450000'
  #     }
  #     expect(described_class.split_postal_code(address)).to eq('firstFive' => '12345', 'lastFour' => '0000')
  #   end

  #   it 'should split the code correctly with 5 digits' do
  #     address = {
  #       'postalCode' => '12345'
  #     }
  #     expect(described_class.split_postal_code(address)).to eq('firstFive' => '12345', 'lastFour' => '')
  #   end
  # end

  # describe '#split_date' do
  #   it 'should return nil with no date' do
  #     expect(described_class.split_date(nil)).to be_nil
  #   end

  #   it 'should return nil if date not correct format (expected yyyy-mm-dd)' do
  #     expect(described_class.split_date('11052018')).to be_nil
  #   end

  #   it 'should split date correctly' do
  #     expect(described_class.split_date('2018-11-05')).to eq('month' => '11', 'day' => '05', 'year' => '2018')
  #   end
  # end

  # describe '#validate_date' do
  #   it 'should return nil with bad data' do
  #     expect(described_class.validate_date('1234567')).to be_nil
  #   end

  #   it 'should return nil with nil date' do
  #     expect(described_class.validate_date(nil)).to be_nil
  #   end

  #   it 'should return nil with impossible date' do
  #     expect(described_class.validate_date('2018-01-32')).to be nil
  #   end

  #   it 'should return nil with blank date' do
  #     expect(described_class.validate_date('')).to be_nil
  #   end

  #   it 'should return date' do
  #     expect(described_class.validate_date('2018-01-01')).to be_truthy
  #   end
  # end
end

...
RSpec.describe EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob, :uploader_helpers,
               skip: 'TODO after schema built' do
  stub_virus_scan
  let(:job) { described_class.new }
  let(:claim) { create(:employment_questionnaires_claim) }
  let(:service) { double('service') }
  let(:monitor) { EmploymentQuestionnaires::Monitor.new }
  ...
  describe '#perform' do
    ...
    before do
      ...
      allow(EmploymentQuestionnaires::SavedClaim).to receive(:find).and_return(claim)
    ...
    context 'with employment_questionnaires_form_enabled flipper' do
      before do
        allow(UserAccount).to receive(:find).and_return(double('user_account'))
      end

      it 'processes claim when flipper is enabled' do
        allow(Flipper).to receive(:enabled?).with(:employment_questionnaires_form_enabled).and_return(true)
        allow(job).to receive(:process_document).and_return(pdf_path)

        expect(EmploymentQuestionnaires::SavedClaim).to receive(:find).and_return(claim)
      ...
      it 'returns early when flipper is disabled' do
        allow(Flipper).to receive(:enabled?).with(:employment_questionnaires_form_enabled).and_return(false)

        expect(EmploymentQuestionnaires::SavedClaim).not_to receive(:find)
    ...
    it 'is unable to find saved_claim_id' do
      allow(EmploymentQuestionnaires::SavedClaim).to receive(:find).and_return(nil)
      ...
      expect { job.perform(claim.id, :user_account_uuid) }.to raise_error(
        EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob::EmploymentQuestionnairesBenefitIntakeError,
        "Unable to find EmploymentQuestionnaires::SavedClaim #{claim.id}"
      )
    end
    # perform
  end
  ...
  describe '#send_submitted_email' do
    ...
    before do
      ...
      allow(EmploymentQuestionnaires::NotificationEmail).to receive(:new).and_return(notification)
    ...
    it 'errors and logs but does not reraise' do
      expect(EmploymentQuestionnaires::NotificationEmail).to receive(:new).with(claim.id)
  ...
  describe 'sidekiq_retries_exhausted block' do
    let(:exhaustion_msg) do
      { 'args' => [], 'class' => 'EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob',
    ...
    context 'when retries are exhausted' do
      it 'logs a distrinct error when no claim_id provided' do
        EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob.within_sidekiq_retries_exhausted_block do
      ...
      it 'logs a distrinct error when only claim_id provided' do
        EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob
          .within_sidekiq_retries_exhausted_block({ 'args' => [claim.id] }) do
            allow(EmploymentQuestionnaires::SavedClaim).to receive(:find).and_return(claim)
            expect(EmploymentQuestionnaires::SavedClaim).to receive(:find).with(claim.id)
      ...
      it 'logs a distrinct error when claim is not found' do
        EmploymentQuestionnaires::BenefitsIntake::SubmitClaimJob
          .within_sidekiq_retries_exhausted_block({ 'args' => [claim.id - 1, 2] }) do
            expect(EmploymentQuestionnaires::SavedClaim).to receive(:find).with(claim.id - 1)

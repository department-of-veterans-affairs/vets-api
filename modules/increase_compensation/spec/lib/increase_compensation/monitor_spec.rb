...
RSpec.describe IncreaseCompensation::Monitor do
  ...
  let(:message_prefix) { "#{described_class} #{IncreaseCompensation::FORM_ID}" }

  describe '#service_name' do
    it 'returns expected name' do
      expect(monitor.send(:service_name)).to eq('increase-compensation')
    end
  end
    ...
    describe '#track_submission_exhaustion' do
      context 'with a claim parameter' do
        it 'logs sidekiq job exhaustion' do
          ...
          expect(IncreaseCompensation::NotificationEmail).to receive(:new).with(claim.id).and_return notification

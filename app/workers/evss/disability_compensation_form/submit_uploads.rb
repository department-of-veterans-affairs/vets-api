    ...
    class SubmitUploads < Job
      ...
      def perform(submission_id, uploads)
        ...
        with_tracking("Form526 Upload: #{guid}", submission.saved_claim_id, submission.id) do
          supporting_evidence_attachment_uploader = SupportingEvidenceAttachment.find_by(guid: guid)&.fetch_file
          file_body = supporting_evidence_attachment_uploader&.read_for_upload
          raise ArgumentError, "supporting evidence attachment with guid #{guid} has no file data" if file_body.nil?

          document_data = create_document_data(upload_data, supporting_evidence_attachment_uploader.final_filename)
      ...
      def create_document_data(upload_data, file_name)

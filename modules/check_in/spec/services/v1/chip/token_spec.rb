...
describe V1::Chip::Token do
  ...
  describe '.build' do
    it 'returns an instance of Token' do
      expect(subject.build).to be_an_instance_of(V1::Chip::Token)
    end
  end
  ...
  describe '#fetch' do
    ...
    before do
      allow_any_instance_of(V1::Chip::Request).to receive(:post).with(anything).and_return(faraday_response)
    end
